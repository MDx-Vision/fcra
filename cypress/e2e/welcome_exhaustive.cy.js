...
describe.skip('/signup/welcome Page Tests', () => {
  ...
  describe.skip('Info Panel Tests', () => {
    ...
    it('should display sample analysis title', () => {
      ...
      cy.get('.info-card-title').first().invoke('text').should('match', /^SAMPLE ANALYSIS$/i);
    });
    ...
    it('should display violations detected header', () => {
      cy.get('.info-panel').invoke('text').should('match', /^VIOLATIONS DETECTED$/i);
    });
    ...
    it('should display FCRA § 611 Dispute violation', () => {
      cy.get('.violation-item').eq(0).within(() => {
        cy.get('.violation-type').should('contain.text', 'FCRA § 611 Dispute');
        cy.get('.violation-desc').should('contain.text', 'Response period exceeded - Bureau failed to respond within 30 days');
      });
    });

    it('should display FCRA § 623 Furnisher violation', () => {
      cy.get('.violation-item').eq(1).within(() => {
        cy.get('.violation-type').should('contain.text', 'FCRA § 623 Furnisher');
    ...
    it('should display potential damages card', () => {
      ...
      cy.get('.info-card-title').eq(1).invoke('text').should('match', /^POTENTIAL DAMAGES$/i);
    });
    ...
    it('should display estimated case value', () => {
      ...
      cy.get('.estimate-label').invoke('text').should('match', /^ESTIMATED CASE VALUE$/i);
