...
describe('Staff Login Page - /dashboard/case-law', () => {
  ...
  describe('UI Element Tests', () => {
    ...
    it('should display client portal link', () => {
      ...
      cy.get('[data-testid="client-portal-link"]').should('contain.text', 'Client Login').and('have.attr', 'href', '/portal/login');
    });
  ...
  describe('Error Handling Tests', () => {
    ...
    it('should handle network errors gracefully', () => {
      cy.intercept('POST', '/staff/login', { forceNetworkError: true }).as('loginError');
      cy.get('[data-testid="email-input"]').type('test@example.com');
      cy.get('[data-testid="password-input"]').type('password123');
      cy.get('[data-testid="login-button"]').click();
    });
