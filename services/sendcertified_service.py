...
from database import SessionLocal, CertifiedMailOrder, IntegrationConnection, IntegrationEvent, Client
...
class SendCertifiedService:
    ...
    def _load_credentials_from_db(self):
        ...
        try:
            connection = session.query(IntegrationConnection).filter_by(
    ...
    def _log_event(self, event_type: str, event_data: dict = None, 
                   client_id: int = None, request_id: str = None,
    ...
    def _update_connection_status(self, status: str, error: str = None):
        ...
        try:
            ...
            if connection:
                ...
                connection.last_connected_at = datetime.utcnow() if status == 'connected' else None
    ...
    def create_mailing(self, recipient: str, address: Dict[str, str], 
                       document_content: bytes, mail_class: str = 'certified',
                       client_id: int = None, dispute_id: int = None,
                       letter_type: str = None, bureau: str = None) -> Dict[str, Any]:
        """
        ...
        payload = {
            'recipient': {
                'name': recipient,
                'address_line_1': address.get('street', address.get('address_line_1', '')),
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                label_id = data.get('label_id') or data.get('id') or data.get('mailing_id')
    ...
    def get_tracking_status(self, tracking_number: str = None, 
        ...
        try:
            ...
            if not self.is_configured():
                order = session.query(CertifiedMailOrder).filter_by(
                    tracking_number=tracking_number
                ).first()
                ...
                if order:
                    return {
                        'success': True,
                        'status': order.status,
                        'events': order.webhook_data.get('events', []) if order.webhook_data else [],
                        'delivered_at': order.delivered_at.isoformat() if order.delivered_at else None,
                        'signed_by': order.webhook_data.get('signed_by') if order.webhook_data else None,
                        'error': None,
                        'cached': True
                    }
    ...
    def download_return_receipt(self, tracking_number: str = None,
                                order_id: int = None) -> Optional[bytes]:
        """
        ...
        try:
            ...
            if not tracking_number:
                logger.warning("No tracking number provided for return receipt download")
                return None
            ...
            if response.status_code == 200:
                ...
                if 'pdf' in content_type.lower() or 'octet-stream' in content_type.lower():
                    ...
                    logger.warning(f"Unexpected content type for receipt: {content_type}")
                    return None
...
def configure_sendcertified(api_key: str, api_secret: str, sandbox: bool = True) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        service = SendCertifiedService(api_key=api_key, api_secret=api_secret, sandbox=sandbox)
...
def get_sendcertified_status() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            'last_connected': connection.last_connected_at.isoformat() if connection.last_connected_at else None,
...
def get_mailing_statistics() -> Dict[str, Any]:
    ...
    try:
        ...
        submitted = session.query(func.count(CertifiedMailOrder.id)).filter(
            CertifiedMailOrder.status == 'submitted'
        ).scalar() or 0
        
        in_transit = session.query(func.count(CertifiedMailOrder.id)).filter(
            CertifiedMailOrder.status.in_(['mailed', 'in_transit', 'out_for_delivery'])
        ).scalar() or 0
        
        delivered = session.query(func.count(CertifiedMailOrder.id)).filter(
            CertifiedMailOrder.status == 'delivered'
        ).scalar() or 0
        
        returned = session.query(func.count(CertifiedMailOrder.id)).filter(
            CertifiedMailOrder.status == 'returned'
        ).scalar() or 0
        
        from datetime import timedelta
        thirty_days_ago = datetime.utcnow() - timedelta(days=30)
        this_month = session.query(func.count(CertifiedMailOrder.id)).filter(
