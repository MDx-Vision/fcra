...
OUTLOOK_REDIRECT_URI = os.getenv(
    ...
    "http://localhost:5001/api/calendar/outlook/callback",
)
...
class CalendarSyncService:
    ...
    def exchange_google_code(self, code: str, staff_id: int) -> CalendarIntegration:
        ...
        existing = (
            ...
            .filter_by(staff_id=staff_id, provider="google")
            .first()
        )
        ...
        if existing:
            ...
            integration = CalendarIntegration(
                ...
                calendar_name=(
                    primary_calendar.get("summary") if primary_calendar else None
                ),
                is_active=True,
            )
    ...
    def exchange_outlook_code(self, code: str, staff_id: int) -> CalendarIntegration:
        ...
        default_calendar = next(
            (c for c in calendars if c.get("isDefaultCalendar")),
            calendars[0] if calendars else None,
        )
        ...
        existing = (
            self.db.query(CalendarIntegration)
            .filter_by(staff_id=staff_id, provider="outlook")
            .first()
        )
        ...
        if existing:
            ...
            integration = CalendarIntegration(
                ...
                calendar_name=(
                    default_calendar.get("name") if default_calendar else None
                ),
                is_active=True,
            )
            self.db.add(integration)
    ...
    def get_integration(
        self, staff_id: int, provider: Optional[str] = None
    ...
    def set_calendar(
        self, integration_id: int, staff_id: int, calendar_id: str, calendar_name: str
    ) -> bool:
    ...
    def get_free_busy(
        ...
        integrations = (
            self.db.query(CalendarIntegration)
            .filter_by(staff_id=staff_id, is_active=True, check_free_busy=True)
            .all()
        )
        ...
        for integration in integrations:
            ...
            try:
                if integration.provider == "google":
                    ...
                    busy = self._outlook_get_free_busy(
                ...
                logger.error(
                    f"Free/busy check failed for integration {integration.id}: {e}"
                )
    ...
    def _google_get_free_busy(
        ...
        return [
            {"start": busy.get("start"), "end": busy.get("end"), "provider": "google"}
    ...
    def sync_booking_to_calendar(self, booking: Booking) -> Optional[CalendarEvent]:
        ...
        try:
            ...
            logger.info(
                f"Booking {booking.id} synced to {integration.provider} calendar"
            )
    ...
    def delete_calendar_event(self, booking_id: int) -> bool:
        ...
        calendar_event = (
            self.db.query(CalendarEvent).filter_by(booking_id=booking_id).first()
        )
    ...
    def update_sync_status(
        self, integration: CalendarIntegration, status: str, error: Optional[str] = None
    ):
