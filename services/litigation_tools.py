...
def calculate_damages(violations, actual_damages_input):
    ...
    damages = {
        ...
        "punitive": {"multiplier": 0, "amount": 0},
        "attorney_fees": {"estimated_hours": 0, "hourly_rate": 250, "total": 0},
        "settlement": {"total_exposure": 0, "target": 0, "minimum": 0},
    }
    ...
    for violation in violations:
        ...
        if (
            "605B" in section_upper
            or "605(B)" in section_upper
            or "1681C" in section_upper
        ):
    ...
    if total_violations > 0:
        ...
        if damages["punitive"]["multiplier"] > 0:
            damages["punitive"]["amount"] = (
                damages["statutory"]["total"] * damages["punitive"]["multiplier"]
            )
...
def assess_willfulness(violation_description, violation_type, bureau_responses=None):
    ...
    """
    willfulness = {"is_willful": False, "indicators": [], "level": "Negligence"}
    ...
    if (
        "documented proof" in description_lower
        and "failed to correct" in description_lower
    ):
        willfulness["is_willful"] = True
