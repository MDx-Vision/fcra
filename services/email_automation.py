...
def trigger_welcome_email(db, client_id):
    ...
    if custom_html:
        html_content = apply_template_merge_tags(
            custom_html,
            {
        ...
        subject = apply_template_merge_tags(
            custom_subject, {"client_name": client.name}
        )
    else:
...
def trigger_document_reminder_email(db, client_id, missing_docs):
    ...
    settings = get_email_settings(db)
    if not settings.get("email_enabled") or not settings.get(
    ...
    portal_url = get_portal_url(client)
    missing_docs_str = (
        ", ".join(missing_docs) if isinstance(missing_docs, list) else str(missing_docs)
    )
    ...
    if custom_html:
        ...
        html_content = email_templates.document_reminder_email(
            client.name, missing_docs, portal_url
        )
...
def trigger_case_update_email(db, client_id, status, details=None):
    ...
    if custom_html:
        ...
        subject = apply_template_merge_tags(
            custom_subject, {"client_name": client.name, "case_status": status_display}
        )
    else:
        html_content = email_templates.case_update_email(
            client.name, status, details, portal_url
        )
...
def trigger_dispute_sent_email(db, client_id, bureau, tracking_info=None):
    ...
    if custom_html:
        ...
        subject = apply_template_merge_tags(
            custom_subject, {"client_name": client.name, "bureau": bureau or ""}
        )
    else:
        html_content = email_templates.dispute_sent_email(
            client.name, bureau, tracking_info, portal_url
        )
...
def trigger_cra_response_email(db, client_id, bureau, result_summary=None):
    ...
    if custom_html:
        ...
        html_content = email_templates.cra_response_email(
            client.name, bureau, result_summary, portal_url
        )
...
def trigger_payment_reminder_email(db, client_id, amount, due_date=None):
    ...
    if custom_html:
        ...
        html_content = email_templates.payment_reminder_email(
            client.name, amount, due_date
        )
...
def trigger_analysis_ready_email(db, client_id, violations_count=None, exposure=None):
    ...
    if custom_html:
        ...
        html_content = email_templates.analysis_ready_email(
            client.name, violations_count, exposure, portal_url
        )
...
def trigger_letters_ready_email(db, client_id, letter_count, bureaus=None):
    ...
    portal_url = get_portal_url(client)
    bureaus_str = (
        ", ".join(bureaus) if isinstance(bureaus, list) else str(bureaus or "")
    )
    ...
    if custom_html:
        ...
        html_content = email_templates.letters_ready_email(
            client.name, letter_count, bureaus, portal_url
        )
