...
from database import Affiliate, BillingPlan, Client, ClientSubscription, Commission
...
class RevenueMetricsService:
    ...
    def get_revenue_by_period(
        self, period: str = "month", limit: int = 12
    ) -> List[Dict[str, Any]]:
        """
        ...
        results = (
            ...
            .filter(Client.total_paid > 0, Client.payment_received_at.isnot(None))
            .group_by(date_trunc)
    ...
    def get_mrr(self) -> Dict[str, Any]:
        ...
        results = (
            ...
            .join(BillingPlan, ClientSubscription.plan_id == BillingPlan.id)
            .filter(
    ...
    def get_mrr_growth(self, months: int = 6) -> List[Dict[str, Any]]:
        ...
        for i in range(months - 1, -1, -1):
            ...
            count = (
                self.db.query(func.count(ClientSubscription.id))
                .filter(
                    ClientSubscription.status == "active",
            ...
            avg_plan_price = (
                self.db.query(func.avg(BillingPlan.price_cents))
                .filter(BillingPlan.billing_interval == "month")
                .scalar()
                or 0
            )
        ...
        active_clients = (
            self.db.query(
                func.avg(
                    extract(
                        "epoch",
                        func.coalesce(Client.updated_at, datetime.utcnow())
                        - Client.created_at,
                    )
                    / (30 * 24 * 60 * 60)
                ).label("avg_months")
    ...
    def get_commission_metrics(
        ...
        # Active affiliates
        active_affiliates = (
            self.db.query(func.count(Affiliate.id))
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        if prev_revenue["total_cents"] > 0:
            revenue_growth = (
                (current_revenue["total_cents"] - prev_revenue["total_cents"])
                / prev_revenue["total_cents"]
            ) * 100
        ...
        new_clients = (
            self.db.query(func.count(Client.id))
            .filter(Client.created_at >= month_ago)
            .scalar()
            or 0
        )
        ...
        new_paid = (
            self.db.query(func.count(Client.id))
            .filter(
                Client.payment_received_at >= month_ago, Client.payment_status == "paid"
            )
            .scalar()
            or 0
        )
        ...
        return {
            ...
            "clients": {
                ...
                "conversion_rate": round(
                    (new_paid / new_clients * 100) if new_clients > 0 else 0, 2
                ),
            },
            "affiliates": {
    ...
    def get_revenue_chart_data(
        ...
        return {
            "labels": labels,
            "datasets": {"revenue": revenue_values, "clients": client_counts},
        }
        ...
        return [
            {
                ...
                "payment_date": (
                    r.payment_received_at.isoformat() if r.payment_received_at else None
                ),
