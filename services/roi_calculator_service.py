...
from database import (
    get_db, Client, Violation, DisputeItem, Analysis, Damages,
    Settlement, ROICalculation, Staff, CreditScoreSnapshot
)
...
class ROICalculatorService:
    ...
    def calculate_roi(
        ...
        actual = self._calculate_actual_damages(
            analysis_damages,
            actual_damages or {}
        )
        ...
        if save:
            calculation = self._save_calculation(
                client_id,
                staff_id,
                result
            )
    ...
    def _get_dispute_items(self, client_id: int) -> List[DisputeItem]:
        ...
        return self.db.query(DisputeItem).filter(
    ...
    def _get_analysis_damages(self, client_id: int) -> Optional[Damages]:
        ...
        if analysis:
            return self.db.query(Damages).filter(
                Damages.analysis_id == analysis.id
            ).first()
        return None
    ...
    def _calculate_statutory_damages(self, violations: List[Violation]) -> Dict[str, float]:
    ...
    def _calculate_actual_damages(
        self,
        analysis_damages: Optional[Damages],
        ...
        if analysis_damages:
            ...
            result['emotional_distress'] = analysis_damages.time_stress_amount or 0  # Mapped from time_stress
    ...
    def _analyze_dispute_items(self, items: List[DisputeItem]) -> Dict[str, Any]:
        ...
        for item in items:
            ...
            elif 'public' in item_type or 'record' in item_type or 'judgment' in item_type or 'lien' in item_type:
    ...
    def _calculate_litigation_potential(
        ...
        if items['deletion_rate'] < 0.3:
            score += 10
            factors.append({'name': 'Low deletion rate (resistant bureaus)', 'points': 10})
            notes.append('Bureaus appear resistant - litigation may be effective')

        # Generate notes
        if score >= 70:
            notes.append('Strong litigation potential - recommend attorney consultation')
        elif score >= 50:
            notes.append('Moderate litigation potential - may benefit from attorney review')
        elif score >= 30:
            notes.append('Some litigation factors present - dispute process may be sufficient')
        else:
    ...
    def _estimate_score_improvement(self, items: Dict[str, Any]) -> int:
        ...
        estimated_deletions = {
            'collections': round(items['by_type']['collections'] * HISTORICAL_DELETION_RATES['collection']),
            'charge_offs': round(items['by_type']['charge_offs'] * HISTORICAL_DELETION_RATES['charge_off']),
            'late_payments': round(items['by_type']['late_payments'] * HISTORICAL_DELETION_RATES['late_payment']),
            'inquiries': round(items['by_type']['inquiries'] * HISTORICAL_DELETION_RATES['inquiry']),
            'public_records': round(items['by_type']['public_records'] * HISTORICAL_DELETION_RATES['public_record']),
        }
        ...
        score_improvement = (
            ...
            estimated_deletions['public_records'] * SCORE_IMPROVEMENT_PER_PUBLIC_RECORD
        )
    ...
    def _save_calculation(
        ...
        calculation = ROICalculation(
            ...
            punitive_damages_potential=result['statutory_damages']['punitive_potential'],
    ...
    def get_client_calculations(
        ...
        calculations = self.db.query(ROICalculation).filter(
            ROICalculation.client_id == client_id
        ).order_by(
            ROICalculation.calculated_at.desc()
        ).limit(limit).all()
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        recent_calcs = self.db.query(func.count(ROICalculation.id)).filter(
            ROICalculation.calculated_at >= thirty_days_ago
        ).scalar() or 0
        ...
        high_litigation = self.db.query(func.count(ROICalculation.id)).filter(
            ROICalculation.litigation_recommended == True
        ).scalar() or 0

        # Total estimated value across all clients
        total_value = self.db.query(
            func.sum(ROICalculation.total_estimated_value)
        ).scalar() or 0

        # Recent calculations for display
        recent = self.db.query(ROICalculation).order_by(
            ROICalculation.calculated_at.desc()
        ).limit(10).all()
    ...
    def get_aggregate_stats(self, period: str = 'all') -> Dict[str, Any]:
        ...
        litigation_recommended = sum(1 for c in calculations if c.litigation_recommended)

        return {
            'count': len(calculations),
            'total_violations': total_violations,
            'total_items': total_items,
            'avg_estimate': round(sum(estimates) / len(estimates), 2) if estimates else 0,
            'max_estimate': max(estimates) if estimates else 0,
            'min_estimate': min(estimates) if estimates else 0,
            'total_estimated_value': round(sum(c.total_estimated_value for c in calculations), 2),
            'litigation_recommended_pct': round(litigation_recommended / len(calculations) * 100, 1) if calculations else 0,
            'by_confidence': {
                'high': sum(1 for c in calculations if c.confidence_level == 'high'),
                'medium': sum(1 for c in calculations if c.confidence_level == 'medium'),
    ...
    def quick_estimate(
        ...
        # Total items
        total_items = (
            collections_count + chargeoffs_count + late_payments_count +
            inquiries_count + public_records_count
        )
    ...
    def export_calculations(
        ...
        for calc in calculations:
            ...
            export_data.append({
                ...
                'calculated_at': calc.calculated_at.isoformat() if calc.calculated_at else '',
                ...
                'litigation_recommended': 'Yes' if calc.litigation_recommended else 'No',
