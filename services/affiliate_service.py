...
from database import (
    get_db, Affiliate, Commission, Client
)
...
def generate_affiliate_code(name: str = None) -> str:
    ...
    session = get_db()
    try:
        existing = session.query(Affiliate).filter(Affiliate.affiliate_code == code).first()
        if existing:
...
def validate_affiliate_code(code: str) -> dict:
    ...
    try:
        affiliate = session.query(Affiliate).filter(
            Affiliate.affiliate_code == code.upper().strip()
        ).first()
...
def process_referral(client_id: int, affiliate_code: str) -> dict:
    ...
    try:
        ...
        if client.referred_by_affiliate_id:
            return {"success": False, "message": "Client already has an affiliate referral"}
...
def calculate_commission(client_id: int, trigger_type: str, amount: float) -> dict:
    ...
    try:
        ...
        affiliate = session.query(Affiliate).filter(
            Affiliate.id == client.referred_by_affiliate_id
        ).first()
        ...
        if affiliate.parent_affiliate_id:
            parent = session.query(Affiliate).filter(
            ...
            if parent:
                ...
                parent.pending_earnings = (parent.pending_earnings or 0) + level_2_amount
...
def get_affiliate_stats(affiliate_id: int) -> dict:
    ...
    try:
        ...
        referred_clients = session.query(Client).filter(
            Client.referred_by_affiliate_id == affiliate_id
        ).all()
        
        converted_clients = [c for c in referred_clients if c.payment_status == 'paid']
        conversion_rate = (len(converted_clients) / len(referred_clients) * 100) if referred_clients else 0
        
        level_1_commissions = session.query(Commission).filter(
            Commission.affiliate_id == affiliate_id,
            Commission.level == 1
        ).all()
        
        level_2_commissions = session.query(Commission).filter(
            Commission.affiliate_id == affiliate_id,
            Commission.level == 2
        ).all()
        ...
        paid_commissions = session.query(Commission).filter(
            Commission.affiliate_id == affiliate_id,
            Commission.status == 'paid'
        ).all()
        
        sub_affiliates = session.query(Affiliate).filter(
...
def process_payout(affiliate_id: int, amount: float, payout_method: str = None, notes: str = None) -> dict:
...
def get_commission_history(affiliate_id: int, limit: int = 50) -> list:
    ...
    try:
        ...
        for c in commissions:
            ...
            client_name = f"{client.first_name or ''} {client.last_name or ''}".strip() if client else "Unknown"
...
def create_affiliate(
    ...
    session = get_db()
    try:
        existing = session.query(Affiliate).filter(Affiliate.email == email.lower().strip()).first()
        if existing:
            return {"success": False, "message": "An affiliate with this email already exists"}
...
def update_affiliate(affiliate_id: int, **kwargs) -> dict:
    ...
    try:
        ...
        allowed_fields = [
            'name', 'email', 'phone', 'company_name', 'parent_affiliate_id',
            'commission_rate_1', 'commission_rate_2', 'status', 'payout_method', 'payout_details'
        ]
...
def get_all_affiliates(status: str = None, limit: int = 100) -> list:
    ...
    try:
        ...
        for a in affiliates:
            referred_clients = session.query(Client).filter(
                Client.referred_by_affiliate_id == a.id
            ).count()
...
def get_affiliate_by_id(affiliate_id: int) -> dict:
    ...
    try:
        ...
        parent = None
        if affiliate.parent_affiliate_id:
            parent_affiliate = session.query(Affiliate).filter(
                Affiliate.id == affiliate.parent_affiliate_id
            ).first()
            if parent_affiliate:
                parent = {
...
def get_dashboard_stats() -> dict:
    ...
    try:
        ...
        active_affiliates = session.query(Affiliate).filter(Affiliate.status == 'active').count()
        pending_affiliates = session.query(Affiliate).filter(Affiliate.status == 'pending').count()
        ...
        total_referrals = session.query(Client).filter(
            Client.referred_by_affiliate_id.isnot(None)
        ).count()
