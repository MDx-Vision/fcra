...
WARNING_LEVELS = {"critical": 30, "warning": 90, "caution": 180}


def calculate_sol(
    violation_date: date, discovery_date: Optional[date] = None
) -> Dict[str, Any]:
    """
    ...
    occurrence_expiration = violation_date + relativedelta(
        years=OCCURRENCE_PERIOD_YEARS
    )
...
def is_expired(violation_date: date, discovery_date: Optional[date] = None) -> bool:
    """
...
def get_violations_with_sol_status(
    db_session, client_id: Optional[int] = None
) -> List[Dict]:
    """
    ...
    """
    from database import Analysis, Client, Violation

    query = (
        db_session.query(Violation, Client, Analysis)
        .join(Client, Violation.client_id == Client.id)
        .join(Analysis, Violation.analysis_id == Analysis.id)
    )
...
def get_upcoming_expirations(db_session, days: int = 90) -> List[Dict]:
    ...
    upcoming = [
        ...
        if v.get("remaining_days") is not None and 0 <= v["remaining_days"] <= days
    ]
...
def check_sol_deadlines(db_session) -> Dict[str, Any]:
    ...
    from database import CaseDeadline, Client, Violation
    ...
    for v_data in violations_needing_warning:
        try:
            ...
            errors.append(
                f"Error processing violation {v_data.get('violation_id')}: {str(e)}"
            )
...
def update_violation_sol_dates(
    ...
    try:
        db_session.commit()
        return {"success": True, "violation_id": violation_id, **sol_info}
    except Exception as e:
        db_session.rollback()
        return {"error": str(e)}
