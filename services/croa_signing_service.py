...
class CROASigningService:
    ...
    def get_or_create_progress(self, client_id: int) -> Optional[CROAProgress]:
        ...
        progress = (
            self.db.query(CROAProgress)
            .filter(CROAProgress.client_id == client_id)
            .first()
        )
    ...
    def get_progress_summary(self, client_id: int) -> Dict[str, Any]:
        ...
        for doc in CROA_DOCUMENTS:
            ...
            template = (
                ...
                .filter(DocumentTemplate.code == doc["code"])
                .first()
            )
        ...
        return {
            ...
            "required_completed": sum(
                1 for d in documents_status if d["is_signed"] and d["is_required"]
            ),
    ...
    def get_document(self, document_code: str) -> Dict[str, Any]:
        ...
        doc_config = next(
            (d for d in CROA_DOCUMENTS if d["code"] == document_code), None
        )
    ...
    def get_current_document(self, client_id: int) -> Dict[str, Any]:
        ...
        return {"success": True, "is_complete": True, "message": "All documents signed"}
    ...
    def sign_document(
        ...
        template = (
            self.db.query(DocumentTemplate)
            .filter(DocumentTemplate.code == document_code)
            .first()
        )
        ...
        return {
            ...
            "is_complete": progress.is_complete,
            "next_document": (
                next_doc.get("document") if next_doc.get("success") else None
            ),
            "cancellation_status": (
    ...
    def skip_optional_document(
        self, client_id: int, document_code: str
    ) -> Dict[str, Any]:
    ...
    def get_cancellation_status(self, client_id: int) -> Dict[str, Any]:
        ...
        return {"success": True, **self._get_cancellation_status(progress)}
    ...
    def can_begin_services(self, client_id: int) -> Dict[str, Any]:
        ...
        return {"success": True, "can_begin": True, "reason": "all_requirements_met"}
    ...
    def _save_signature_image(
        self, client_id: int, document_code: str, signature_data: str
    ) -> Optional[str]:
