...
from database import get_db, APIKey, APIRequest, APIWebhook, API_SCOPES, WEBHOOK_EVENTS
...
class RateLimiter:
    ...
    def check_and_increment(self, key_id: int, per_minute: int, per_day: int) -> Tuple[bool, dict]:
        """
...
class APIAccessService:
    ...
    def revoke_api_key(self, key_id: int, staff_id: Optional[int] = None) -> Dict[str, Any]:
        """Revoke an API key"""
        db, should_close = self._get_db()
        try:
    ...
    def rotate_api_key(self, key_id: int, staff_id: Optional[int] = None) -> Dict[str, Any]:
        """
    ...
    def check_rate_limit(self, key_id: int, per_minute: int = 60, per_day: int = 10000) -> Tuple[bool, dict]:
        """
        ...
        try:
            ...
            if request_body:
                safe_body = {k: v for k, v in request_body.items()
                           if k.lower() not in ['password', 'secret', 'token', 'key', 'ssn', 'credit_card']}
    ...
    def get_key_usage_stats(self, key_id: int, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            requests = db.query(APIRequest).filter(
                APIRequest.api_key_id == key_id,
                APIRequest.created_at >= start_date
            ).all()
            
            total_requests = len(requests)
            successful_requests = sum(1 for r in requests if 200 <= r.response_status < 300)
            error_requests = sum(1 for r in requests if r.response_status >= 400)
            avg_response_time = sum(r.response_time_ms or 0 for r in requests) / total_requests if total_requests > 0 else 0
    ...
    def list_api_keys(self, staff_id: Optional[int] = None, tenant_id: Optional[int] = None) -> List[Dict]:
    ...
    def trigger_webhook(self, event_type: str, payload: dict, tenant_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            query = db.query(APIWebhook).filter(
                APIWebhook.is_active == True
            )
            ...
            for webhook in webhooks:
                ...
                signature = self.generate_webhook_signature(payload_json, webhook.secret, timestamp)
    ...
    def verify_webhook_signature(self, payload: str, signature: str, secret: str, timestamp: int, max_age_seconds: int = 300) -> bool:
    ...
    def get_api_documentation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "servers": [
                {"url": f"{base_url}/api/v1", "description": "Production API"}
            ],
            "security": [
                {"bearerAuth": []}
            ],
                ...
                "schemas": {
                    "Client": {
                        ...
                        "properties": {
                            ...
                            "phone": {"type": "string"},
                            "status": {"type": "string", "enum": ["signup", "active", "paused", "complete", "cancelled"]},
                                ...
                                "application/json": {
                                    "schema": {
                                        ...
                                        "properties": {
                                            ...
                                            "dispute_round": {"type": "integer", "default": 1}
                                        }
                                    }
                                }
                            }
                                    ...
                                    "schema": {
                                        ...
                                        "properties": {
                                            ...
                                            "events": {
                                                ...
                                                "items": {
                                                    "type": "string",
                                                    "enum": ["client.created", "client.updated", "case.created", "case.status_changed", "dispute.created", "dispute.status_changed", "analysis.completed", "document.generated", "settlement.updated"]
                                                }
                                            }
                                        }
                                    }
