...
from database import Client, SessionLocal, TimelineEvent
...
class TimelineService:
    ...
    def create_event(
        ...
        event = TimelineEvent(
            ...
            title=title
            or event_config.get("title", event_type.replace("_", " ").title()),
            ...
            is_milestone=(
                ...
                else event_config.get("is_milestone", False)
            ),
            is_visible=True,
    ...
    def get_timeline(
        ...
        query = self.db.query(TimelineEvent).filter(
            TimelineEvent.client_id == client_id, TimelineEvent.is_visible == True
        )
        ...
        total = query.count()
        events = (
            query.order_by(desc(TimelineEvent.event_date))
            .offset(offset)
            .limit(limit)
            .all()
        )
    ...
    def get_progress_summary(self, client_id: int) -> Dict[str, Any]:
        ...
        events = (
            self.db.query(TimelineEvent)
            .filter(TimelineEvent.client_id == client_id)
            .all()
        )
        ...
        return {
            ...
            "first_event": (
                min(e.event_date for e in events).isoformat() if events else None
            ),
            "last_event": (
                max(e.event_date for e in events).isoformat() if events else None
            ),
    ...
    def _get_stage_progress(self, event_types: set) -> List[Dict[str, Any]]:
        ...
        stages = [
            {"key": "signup", "name": "Account Created", "check_event": "signup"},
            {
                "key": "onboarding",
                "name": "Onboarding",
                "check_event": "onboarding_complete",
            },
            {
                "key": "report_uploaded",
                "name": "Credit Report Uploaded",
                "check_event": "credit_report_uploaded",
            },
            {
                "key": "analysis_complete",
                "name": "Analysis Complete",
                "check_event": "analysis_complete",
            },
            {
                "key": "disputes_sent",
                "name": "Disputes Sent",
                "check_event": "dispute_sent",
            },
            {
                "key": "awaiting_response",
                "name": "Awaiting Response",
                "check_event": "response_received",
            },
            {
                "key": "completed",
                "name": "Case Resolved",
                "check_event": "case_resolved",
            },
        ]
    ...
    def hide_event(self, event_id: int) -> Dict[str, Any]:
        ...
        event = (
            self.db.query(TimelineEvent).filter(TimelineEvent.id == event_id).first()
        )
        if not event:
    ...
    def backfill_events(self, client_id: int) -> Dict[str, Any]:
        ...
        if not existing and client.created_at:
            self.create_event(
                client_id=client_id, event_type="signup", event_date=client.created_at
            )
            events_created += 1
        ...
        if client.agreement_signed:
            ...
            if not existing:
                self.create_event(
                    ...
                    event_date=getattr(client, "agreement_signed_at", None)
                    or client.updated_at
                    or datetime.utcnow(),
                )
                events_created += 1
        ...
        if client.payment_status in ["paid", "completed", "active"]:
            ...
            if not existing:
                self.create_event(client_id=client_id, event_type="payment_completed")
                events_created += 1
...
def log_document_uploaded(
    db: Session, client_id: int, document_type: str, document_id: int = None
) -> None:
...
def log_analysis_complete(
    db: Session, client_id: int, analysis_id: int, violations_count: int = 0
) -> None:
    ...
    service.create_event(
        ...
        description=(
            f"Found {violations_count} potential violations"
            if violations_count
            else None
        ),
...
def log_response_received(
    db: Session, client_id: int, bureau: str, response_id: int = None
) -> None:
...
def log_status_changed(
    db: Session, client_id: int, old_status: str, new_status: str
) -> None:
    ...
    service.create_event(
        ...
        description=(
            f"Changed from {old_status.replace('_', ' ').title()}"
            if old_status
            else None
        ),
...
def log_milestone(
    db: Session, client_id: int, event_type: str, title: str, description: str = None
) -> None:
    ...
    service = TimelineService(db)
    service.create_event(
        client_id=client_id, event_type=event_type, title=title, description=description
    )
