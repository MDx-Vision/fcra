...
class BasePDFBuilder:
    ...
    def add_cover_page(self, story, title, client_name, report_date=None):
        ...
        story.append(
            Paragraph(f"<b>{self.sanitize_text(client_name)}</b>", self.title_style)
        )
    ...
    def add_numbered_list(self, story, items):
        ...
        for i, item in enumerate(items, 1):
            story.append(
                Paragraph(f"{i}. {self.sanitize_text(item)}", self.bullet_style)
            )
    ...
    def add_table(self, story, data, col_widths=None):
        ...
        sanitized_data = [
            [self.sanitize_text(str(cell)) for cell in row] for row in data
        ]
    ...
    def add_info_box(self, story, title, content, box_color=None):
        ...
        box_data = [
            [Paragraph(f"<b>{self.sanitize_text(title)}</b>", self.heading3_style)],
