...
class RoundLetterBuilder(BasePDFBuilder):
    ...
    def _add_letter_header(self, story, client_data, bureau):
        ...
        if address:
            ...
            self.add_paragraph(story, f"{address.get('city', '')}, {address.get('state', '')} {address.get('zip', '')}")
    ...
    def _add_r1_body(self, story, client_data, bureau):
        ...
        if bureau_violations:
            for i, v in enumerate(bureau_violations, 1):
                self.add_section_header(story, f"Item {i}: {v.get('account_name', 'Unknown Account')}", level=3)
            ...
            self.add_paragraph(story, "[No specific violations identified for this bureau - review required]")
    ...
    def _add_r2_body(self, story, client_data, bureau):
        ...
        violations = client_data.get('violations', [])
        bureau_violations = [v for v in violations if v.get('bureau', '').lower() == bureau.lower()]
        previous_dispute_date = client_data.get('previous_dispute_date', 'previous dispute')
        ...
        self.add_section_header(story, "METHOD OF VERIFICATION DEMAND (Cushman v. TransUnion)", level=2)
        ...
        if bureau_violations:
            self.add_section_header(story, "ITEMS REQUIRING METHOD OF VERIFICATION", level=2)
            
            for i, v in enumerate(bureau_violations[:10], 1):
                self.add_paragraph(story, f"{i}. {v.get('account_name', 'Unknown')} - {v.get('violation_type', 'Disputed')}")
    ...
    def _add_r3_body(self, story, client_data, bureau):
        ...
        if bureau_violations:
            for i, v in enumerate(bureau_violations[:8], 1):
                self.add_paragraph(story, f"{i}. {v.get('account_name', 'Unknown')} - {v.get('fcra_section', 'N/A')}: {v.get('violation_type', '')}")
    ...
    def _add_r4_body(self, story, client_data, bureau):
        ...
        violations_count = len([v for v in violations if v.get('bureau', '').lower() == bureau.lower()])
        self.add_paragraph(story, f"\nNumber of documented FCRA violations: {violations_count}")
    ...
    def _add_cfpb_header(self, story, client_data):
        ...
        self.add_paragraph(story, f"Complainant: {client_data.get('client_name', 'Consumer')}")
    ...
    def _add_cfpb_body(self, story, client_data, bureau):
        ...
        self.add_paragraph(story, "Credit Reporting - Improper use of your report / Problem with investigation")
        ...
        bureau_violations = [v for v in violations if v.get('bureau', '').lower() == bureau.lower()]
        for i, v in enumerate(bureau_violations[:10], 1):
            self.add_paragraph(story, f"{i}. {v.get('account_name', 'Unknown')} - {v.get('fcra_section', 'N/A')}")
