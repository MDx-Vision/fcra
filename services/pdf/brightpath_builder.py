...
class BrightpathPDF(FPDF):
    ...
    def header(self):
        ...
        if HAS_GRADIENTS:
            try:
                gradient = LinearGradient(
                    self,
                    from_x=0, from_y=start_y,
                    to_x=self.w, to_y=start_y,
        ...
        self.cell(0, 10, "BRIGHTPATH ASCEND GROUP", align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        self.set_font('Helvetica', '', 9)
        self.cell(0, 5, "Consumer Protection & FCRA Litigation Specialists", align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    def footer(self):
        ...
        self.cell(0, 4, "Brightpath Ascend Group | Consumer Protection & FCRA Litigation", align='C')
    ...
    def add_document_title(self, title, subtitle=None):
        ...
        if subtitle:
            ...
            self.set_text_color(*BrightpathColors.MEDIUM_GRAY)
            self.multi_cell(0, 6, subtitle, align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    def add_section_header(self, title, with_gradient=True):
        ...
        if with_gradient and HAS_GRADIENTS:
            try:
                ...
                gradient = LinearGradient(
                    self,
                    from_x=20, from_y=start_y,
                    to_x=self.w - 20, to_y=start_y,
                    colors=[BrightpathColors.TEAL_HEX, BrightpathColors.LIME_HEX]
                )
                with self.use_pattern(gradient):
    ...
    def add_cost_table(self, items, title="ESTIMATED COSTS"):
        ...
        self.cell(cost_width, 8, "Cost", border=1, fill=True, align='R', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        
        self.set_font('Helvetica', '', 10)
        for desc, cost in items:
            self.set_x(20)
            self.cell(desc_width, 7, desc, border=1, new_x=XPos.RIGHT)
            self.cell(cost_width, 7, cost, border=1, align='R', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    def add_signature_block(self, name="", date="", title=""):
        ...
        self.set_font('Helvetica', '', 10)
        self.set_x(20)
        self.cell(80, 6, name if name else "Signature", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        self.set_x(20)
        self.set_font('Helvetica', '', 10)
        self.cell(80, 6, f"Date: {date if date else '_______________'}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
...
def create_action_plan_pdf(
    ...
    pdf.add_document_title(
        f"{client_name.upper()} CASE ACTION PLAN",
        "Timeline & Checklist"
    )
    ...
    if case_info.get('case_number'):
        ...
        pdf.cell(0, 6, f"Case No. {case_info['case_number']}", align='C', new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    ...
    if hearing_tasks:
        pdf.add_checklist(hearing_tasks, title=f"HEARING DAY: {case_info.get('hearing_date', '')}")
...
def create_case_summary_pdf(
    ...
    client_info = [
        ...
        ("Analysis Date", case_data.get('analysis_date', datetime.now().strftime('%B %d, %Y'))),
