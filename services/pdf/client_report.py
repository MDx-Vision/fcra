...
class ClientReportBuilder(BasePDFBuilder):
    ...
    def generate(self, output_path, client_data):
        ...
        report_date = client_data.get('report_date', datetime.now().strftime("%B %d, %Y"))
    ...
    def _add_cover_page(self, story, client_name, report_date):
        ...
        self.add_cover_page(story, "COMPREHENSIVE CREDIT ANALYSIS REPORT", client_name, report_date)
    ...
    def _add_table_of_contents(self, story):
        ...
        toc_items = [
            ...
            ("2. Understanding Your FCRA Rights", "Your legal protections under federal law"),
    ...
    def _add_executive_summary(self, story, client_data):
        ...
        self.add_paragraph(story, f"Dear {client_data.get('client_name', 'Valued Client').split()[0]},")
        ...
        findings_data = [
            ...
            ["Case Strength Score", f"{case_score.get('total_score', 'N/A')}/10" if case_score else "N/A"],
            ["Standing for Lawsuit", "Established" if standing.get('has_concrete_harm') else "Needs Documentation"],
        ]
        self.add_table(story, findings_data, col_widths=[3*inch, 2.5*inch])
        
        self.add_section_header(story, "Standing Analysis (Post-TransUnion v. Ramirez)", level=2)
        ...
        if standing:
            ...
            self.add_paragraph(story, "Standing analysis not yet completed. See Section 4 for details.")
        ...
        recommendation = case_score.get('recommendation', 'proceed_with_disputes') if case_score else 'proceed_with_disputes'
    ...
    def _add_fcra_rights(self, story):
        ...
        self.add_section_header(story, "SECTION 2: UNDERSTANDING YOUR FCRA RIGHTS", level=1)
        ...
        self.add_table(story, sections_data, col_widths=[1.5*inch, 2.5*inch, 1.5*inch])
    ...
    def _add_bureau_analysis(self, story, client_data):
        ...
        for bureau in bureaus:
            ...
            bureau_violations = [v for v in violations if v.get('bureau', '').lower() == bureau.lower()]
            
            self.add_paragraph(story, f"<b>Credit Score:</b> {score}")
            self.add_paragraph(story, f"<b>Violations Found:</b> {len(bureau_violations)}")
            ...
            if bureau_violations:
                ...
                self.add_table(story, violation_data, col_widths=[2*inch, 1.2*inch, 2*inch])
            else:
    ...
    def _add_violation_categories(self, story, client_data):
        ...
        for section, section_violations in sorted(by_section.items()):
            self.add_section_header(story, f"FCRA {section} Violations ({len(section_violations)} found)", level=2)
            ...
            for i, v in enumerate(section_violations, 1):
                self.add_section_header(story, f"{i}. {v.get('account_name', 'Unknown Account')}", level=3)
    ...
    def _add_account_analysis(self, story, client_data):
        ...
        self.add_section_header(story, "SECTION 5: ACCOUNT-BY-ACCOUNT ANALYSIS", level=1)
        ...
        for account, account_violations in sorted(by_account.items()):
            ...
            bureaus_affected = list(set(v.get('bureau', 'Unknown') for v in account_violations))
            self.add_paragraph(story, f"<b>Bureaus Affected:</b> {', '.join(bureaus_affected)}")
            self.add_paragraph(story, f"<b>Total Violations:</b> {len(account_violations)}")
            
            issues = list(set(v.get('violation_type', 'Unknown') for v in account_violations))
    ...
    def _add_damages_assessment(self, story, client_data):
        ...
        if damages:
            stat_data = [
                ...
                ["Section 605 Violations", str(damages.get('section_605b_count', 0)), self.format_currency(damages.get('section_605b_amount', 0))],
                ["Section 607(b) Violations", str(damages.get('section_607b_count', 0)), self.format_currency(damages.get('section_607b_amount', 0))],
                ["Section 611 Violations", str(damages.get('section_611_count', 0)), self.format_currency(damages.get('section_611_amount', 0))],
                ["Section 623 Violations", str(damages.get('section_623_count', 0)), self.format_currency(damages.get('section_623_amount', 0))],
                ["TOTAL STATUTORY", "", self.format_currency(damages.get('statutory_damages_total', 0))],
            ]
            self.add_table(story, stat_data, col_widths=[2.5*inch, 1*inch, 1.5*inch])
        ...
        if damages:
            actual_data = [
                ["Type", "Amount"],
                ["Credit Denials", self.format_currency(damages.get('credit_denials_amount', 0))],
                ["Higher Interest Rates", self.format_currency(damages.get('higher_interest_amount', 0))],
                ["Credit Monitoring Costs", self.format_currency(damages.get('credit_monitoring_amount', 0))],
                ["Time & Stress", self.format_currency(damages.get('time_stress_amount', 0))],
                ["Other Actual Damages", self.format_currency(damages.get('other_actual_amount', 0))],
                ["TOTAL ACTUAL", self.format_currency(damages.get('actual_damages_total', 0))],
            ]
        ...
        if damages and willful_count > 0:
            ...
            self.add_paragraph(story, "No willful violations identified for punitive damages at this time.")
        ...
        if damages:
            ...
            self.add_info_box(story, "TOTAL POTENTIAL RECOVERY", 
    ...
    def _add_strategic_recommendations(self, story, client_data):
        ...
        high_value = sorted(violations, key=lambda x: x.get('statutory_damages_max', 0), reverse=True)[:5]
        
        if high_value:
            self.add_paragraph(story, "Focus dispute efforts on these high-value violations:")
