...
class InternalMemoBuilder(BasePDFBuilder):
    ...
    def _add_quick_assessment(self, story, case_data):
        ...
        case_strength = (
            case_score.get("case_strength", "Unknown") if case_score else "Unknown"
        )
        ...
        metrics_data = [
            ...
            [
                ...
                (
                    ...
                    else "Moderate" if total_violations >= 5 else "Low"
                ),
            ],
            [
                "Willful Violations",
                ...
                (
                    "Excellent"
                    if willful_count >= 3
                    else "Good" if willful_count >= 1 else "None"
                ),
            ],
            [
                "Total Exposure",
                self.format_currency(total_exposure),
                (
                    "High"
                    if total_exposure >= 50000
                    else "Moderate" if total_exposure >= 10000 else "Low"
                ),
            ],
            [
                "Standing",
                "Established" if standing.get("has_concrete_harm") else "Needs Work",
                "Ready" if standing.get("has_concrete_harm") else "Action Required",
            ],
        ]
    ...
    def _add_standing_analysis(self, story, case_data):
        ...
        self.add_section_header(
            story, "2. STANDING ANALYSIS (Post-TransUnion v. Ramirez)", level=2
        )
        ...
        if not standing:
            self.add_paragraph(
                story,
                "No standing data available. Complete Stage 1 analysis first.",
                self.warning_style,
            )
            return
        ...
        self.add_table(
            story, standing_data, col_widths=[1.5 * inch, 0.8 * inch, 3.2 * inch]
        )

        has_all_three = (
            standing.get("has_concrete_harm")
            and standing.get("has_dissemination")
            and standing.get("has_causation")
        )

        if has_all_three:
            self.add_paragraph(
                story,
                "STANDING ESTABLISHED - All three elements present. Case ready for litigation track.",
            ...
            self.add_paragraph(
                story,
                f"ACTION REQUIRED: Need {', '.join(missing)} before litigation.",
                self.warning_style,
            )
    ...
    def _add_violation_breakdown(self, story, case_data):
        ...
        self.add_section_header(
            story, "3. VIOLATION BREAKDOWN BY FCRA SECTION", level=2
        )
        ...
        for section in sorted(by_section.keys()):
            ...
            min_damages = sum(
                v.get("statutory_damages_min", 100) for v in section_violations
            )
            max_damages = sum(
                v.get("statutory_damages_max", 1000) for v in section_violations
            )
        ...
        if len(breakdown_data) > 1:
            self.add_table(
                story,
                breakdown_data,
                col_widths=[1.5 * inch, 1 * inch, 1 * inch, 2 * inch],
            )
        else:
    ...
    def _add_willfulness_indicators(self, story, case_data):
        ...
        self.add_section_header(
            story, "5. WILLFULNESS INDICATORS (Safeco v. Burr Standard)", level=2
        )
        ...
        if willful:
            self.add_paragraph(
                story, f"Found {len(willful)} violations with willfulness indicators:"
            )

            for v in willful[:5]:
                notes = v.get("willfulness_notes", "Indicators not specified")
                self.add_paragraph(
                    story, f"* {v.get('account_name', 'Unknown')}: {notes[:100]}"
                )
    ...
    def _add_damages_calculation(self, story, case_data):
        ...
        damages_data = [
            ...
            [
                "Punitive Damages",
                self.format_currency(damages.get("punitive_damages_amount", 0)),
            ],
            [
                "Attorney Fees (Est.)",
                self.format_currency(damages.get("attorney_fees_projection", 0)),
            ],
            ["TOTAL EXPOSURE", self.format_currency(damages.get("total_exposure", 0))],
            [
                "Settlement Target (65%)",
                self.format_currency(damages.get("settlement_target", 0)),
            ],
            [
                "Minimum Acceptable (50%)",
                self.format_currency(damages.get("minimum_acceptable", 0)),
            ],
        ]
    ...
    def _add_red_flags(self, story, case_data):
        ...
        if not standing.get("has_concrete_harm"):
            red_flags.append(
                "Missing concrete harm documentation - critical for standing"
            )
        ...
        if len(violations) < 3:
            red_flags.append(
                f"Low violation count ({len(violations)}) - may not justify litigation costs"
            )

        willful_count = sum(1 for v in violations if v.get("is_willful", False))
        if willful_count == 0:
            red_flags.append(
                "No willful violations - limited punitive damages potential"
            )

        total_score = case_score.get("total_score", 0) if case_score else 0
        if total_score < 5:
            red_flags.append(
                f"Low case score ({total_score}/10) - consider additional documentation"
            )
        ...
        if red_flags:
            ...
            self.add_paragraph(
                story,
                "No significant red flags identified. Case appears solid.",
                self.body_blue_style,
            )
    ...
    def _add_strategy_recommendations(self, story, case_data):
        ...
        recommendation = (
            case_score.get("recommendation", "standard_disputes")
            if case_score
            else "standard_disputes"
        )
