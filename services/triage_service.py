...
def calculate_priority_score(violations, standing, damages, case_score):
    ...
    if standing:
        ...
        if standing.has_dissemination:
            ...
            confidence_factors += 1
        if (
            standing.adverse_action_notices_count
            and standing.adverse_action_notices_count > 0
        ):
            score += 0.5

    if damages:
        total_damages = (damages.actual_damages_total or 0) + (
            damages.statutory_damages_total or 0
        )
...
def determine_complexity(violations, standing, damages):
    ...
    if damages:
        total = (damages.actual_damages_total or 0) + (
            damages.punitive_damages_amount or 0
        )
        if total > 50000:
            complexity_score += 2
        elif total > 25000:
...
def identify_risk_factors(violations, standing, damages):
    ...
    elif (damages.actual_damages_total or 0) + (
        damages.statutory_damages_total or 0
    ) < 1000:
        risks.append(
            {
...
def identify_strengths(violations, standing, damages, case_score):
    ...
    if len(bureaus) >= 3:
        strengths.append(
            {"type": "multi_bureau", "description": f"Violations across all 3 bureaus"}
        )
    ...
    if (
        standing
        and standing.has_concrete_harm
        and (standing.denial_letters_count or 0) > 0
    ):
        strengths.append(
            {
...
def generate_triage_summary(
    ...
    if strengths:
        summary_parts.append(
            f"Strengths: {', '.join(s['description'] for s in strengths[:3])}"
        )
    ...
    if risks:
        ...
        if high_risks:
            summary_parts.append(
                f"⚠️ Risks: {', '.join(r['description'] for r in high_risks)}"
            )
...
def triage_case(analysis_id):
    ...
    try:
        ...
        existing_triage = (
            db.query(CaseTriage).filter(CaseTriage.analysis_id == analysis_id).first()
        )
        ...
        standing = (
            db.query(Standing).filter(Standing.analysis_id == analysis_id).first()
        )
        damages = db.query(Damages).filter(Damages.analysis_id == analysis_id).first()
        case_score = (
            db.query(CaseScore).filter(CaseScore.analysis_id == analysis_id).first()
        )
        ...
        recommended_queue = recommend_queue(
            priority_score, complexity_level, has_critical
        )
...
def get_triage_by_id(triage_id):
    ...
    try:
        ...
        return {
            ...
            "reviewed_at": (
                triage.reviewed_at.isoformat() if triage.reviewed_at else None
            ),
...
def get_queue_cases(queue_name=None, limit=50):
    ...
    try:
        query = db.query(CaseTriage).order_by(
            CaseTriage.priority_score.desc(), CaseTriage.created_at.desc()
        )
        ...
        for triage in cases:
            ...
            analysis = (
                db.query(Analysis).filter(Analysis.id == triage.analysis_id).first()
            )
            ...
            result.append(
                {
                    ...
                    "final_priority": triage.final_priority,
                    "created_at": (
                        triage.created_at.isoformat() if triage.created_at else None
                    ),
...
def get_triage_stats():
    ...
    try:
        ...
        fast_track = (
            db.query(CaseTriage)
            .filter(CaseTriage.recommended_queue == "fast_track")
            .count()
        )

        standard = (
            db.query(CaseTriage)
            .filter(CaseTriage.recommended_queue == "standard")
            .count()
        )

        review_needed = (
            db.query(CaseTriage)
            .filter(CaseTriage.recommended_queue == "review_needed")
            .count()
        )

        hold = (
            db.query(CaseTriage).filter(CaseTriage.recommended_queue == "hold").count()
        )

        unreviewed = db.query(CaseTriage).filter(CaseTriage.reviewed == False).count()

        high_priority = (
            db.query(CaseTriage).filter(CaseTriage.priority_score >= 4).count()
        )
    ...
    try:
        ...
        if final_priority is not None:
            ...
            triage.recommended_queue = recommend_queue(
                triage.final_priority, triage.complexity_level
            )
        else:
