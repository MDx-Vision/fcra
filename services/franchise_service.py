...
class FranchiseService:
    ...
    def create_organization(
        ...
        **kwargs,
        ...
        if org_type not in FRANCHISE_ORG_TYPES:
            raise ValueError(
                f"Invalid org_type. Must be one of: {list(FRANCHISE_ORG_TYPES.keys())}"
            )

        if parent_org_id:
            parent = (
                self.db.query(FranchiseOrganization).filter_by(id=parent_org_id).first()
            )
            if not parent:
                raise ValueError(
    ...
    def update_organization(
        self, org_id: int, **kwargs
    ) -> Optional[FranchiseOrganization]:
        """
        ...
        if "slug" in kwargs and kwargs["slug"] != org.slug:
            existing = (
                self.db.query(FranchiseOrganization)
                .filter(
            ...
            if existing:
                raise ValueError(
                    f"Organization with slug '{kwargs['slug']}' already exists"
                )
    ...
    def delete_organization(self, org_id: int) -> bool:
        ...
        children = (
            self.db.query(FranchiseOrganization).filter_by(parent_org_id=org_id).all()
        )
        if children:
            raise ValueError(
    ...
    def get_organization_hierarchy(
        ...
        def build_tree(org: FranchiseOrganization) -> Dict[str, Any]:
            ...
            children = (
                self.db.query(FranchiseOrganization)
                .filter_by(parent_org_id=org.id, is_active=True)
    ...
    def get_child_organizations(
        self, org_id: int, recursive: bool = False
    ) -> List[FranchiseOrganization]:
        """
        ...
        direct_children = (
            self.db.query(FranchiseOrganization)
            .filter_by(parent_org_id=org_id, is_active=True)
            .order_by(FranchiseOrganization.name)
            .all()
        )
    ...
    def add_member(
        ...
        if role not in FRANCHISE_MEMBER_ROLES:
            raise ValueError(
                f"Invalid role. Must be one of: {list(FRANCHISE_MEMBER_ROLES.keys())}"
            )

        existing = (
            self.db.query(OrganizationMembership)
            .filter_by(organization_id=org_id, staff_id=staff_id)
            .first()
        )
        if existing:
    ...
    def update_member(
        self, org_id: int, staff_id: int, **kwargs
    ...
    def assign_client_to_org(
        self, client_id: int, org_id: int, assigned_by_staff_id: Optional[int] = None
    ) -> OrganizationClient:
        """
        ...
        existing = (
            ...
            .filter_by(organization_id=org_id, client_id=client_id)
            .first()
        )
        if existing:
    ...
    def get_organization_clients(
        self, org_id: int, include_child_orgs: bool = False
        ...
        return (
            self.db.query(OrganizationClient)
            .filter(OrganizationClient.organization_id.in_(org_ids))
            .all()
        )
    ...
    def transfer_client(
        ...
        client_assignment = (
            self.db.query(OrganizationClient)
            .filter_by(organization_id=from_org_id, client_id=client_id)
            .first()
        )
        ...
        if transfer_type not in valid_transfer_types:
            raise ValueError(
                f"Invalid transfer_type. Must be one of: {valid_transfer_types}"
            )

        pending_transfer = (
            self.db.query(InterOrgTransfer)
    ...
    def approve_transfer(
        self, transfer_id: int, approved_by_staff_id: int, approve: bool = True
    ) -> InterOrgTransfer:
        """
    ...
    def get_pending_transfers(
        self, org_id: Optional[int] = None, direction: str = "both"
    ...
    def get_org_stats(
        self, org_id: int, include_children: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        if client_ids:
            ...
            active_cases = len(
                [c for c in cases if c.status in ["active", "in_progress", "pending"]]
            )

            settlements = (
                self.db.query(func.sum(Settlement.final_amount))
                .filter(
        ...
        pending_incoming = (
            self.db.query(InterOrgTransfer)
            .filter_by(to_org_id=org_id, status="pending")
            .count()
        )

        pending_outgoing = (
            self.db.query(InterOrgTransfer)
            .filter_by(from_org_id=org_id, status="pending")
            .count()
        )
    ...
    def get_org_revenue_report(
        self, org_id: int, period: str = "month", include_children: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def calculate_revenue_share(
        self, org_id: int, period: str = "month"
    ) -> Dict[str, Any]:
        """
        ...
        revenue_report = self.get_org_revenue_report(
            org_id, period, include_children=False
        )
    ...
    def get_user_organizations(self, staff_id: int) -> List[Dict[str, Any]]:
        ...
        """
        memberships = (
            self.db.query(OrganizationMembership).filter_by(staff_id=staff_id).all()
        )
    ...
    def check_org_permission(self, staff_id: int, org_id: int, permission: str) -> bool:
        """
    ...
    def get_consolidated_report(self, org_id: int) -> Dict[str, Any]:
        ...
        if client_ids:
            ...
            completed_cases = len(
                [c for c in cases if c.status in ["completed", "settled", "closed"]]
            )
    ...
    def check_org_limits(self, org_id: int) -> Dict[str, Any]:
        ...
        current_clients = (
            self.db.query(OrganizationClient).filter_by(organization_id=org_id).count()
        )
        ...
        return {
            ...
            "users": {
                ...
                "usage_percent": (
                    round((current_users / max_users) * 100, 1) if max_users > 0 else 0
                ),
            },
            "clients": {
                ...
                "usage_percent": (
                    round((current_clients / max_clients) * 100, 1)
                    if max_clients > 0
                    else 0
                ),
            },
...
def get_org_filter(db, staff_user) -> Optional[List[int]]:
    ...
    """
    from database import FranchiseOrganization, OrganizationMembership, Staff
    ...
    if not accessible_orgs:
        primary_membership = (
            ...
            .filter_by(staff_id=staff_user.id, is_primary=True)
            .first()
        )
        ...
        any_membership = (
            db.query(OrganizationMembership).filter_by(staff_id=staff_user.id).first()
        )
...
def get_clients_for_org(db, org_ids: Optional[List[int]]) -> Optional[List[int]]:
    ...
    client_assignments = (
        db.query(OrganizationClient.client_id)
        .filter(OrganizationClient.organization_id.in_(org_ids))
        .all()
    )
    ...
    unassigned = db.query(Client.id).filter(Client.organization_id.is_(None)).all()
