...
from database import SessionLocal, NotarizeTransaction, IntegrationEvent, IntegrationConnection
...
class NotarizeService:
    ...
    def __init__(self, api_key: Optional[str] = None, sandbox: bool = True):
        ...
        """
        self.api_key = api_key or os.environ.get('NOTARIZE_API_KEY') or os.environ.get('PROOF_API_KEY')
        ...
        try:
            ...
            if not connection:
                connection = IntegrationConnection(
                    ...
                    connection_status='configured' if self.is_configured else 'not_configured'
                )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                logger.warning(f"Notarize.com connection test returned {response.status_code}")
    ...
    def create_transaction(
        ...
        try:
            ...
            access_link = (
                response_data.get('access_link') or 
                response_data.get('session_link') or 
                response_data.get('signerUrl') or
            ...
            transaction = NotarizeTransaction(
                ...
                webhook_events=[{'event': 'created', 'timestamp': datetime.utcnow().isoformat()}]
            )
            ...
            logger.info(f"Created notarization transaction {transaction.id} with external ID {transaction_id}")
    ...
    def get_transaction_status(
        ...
        session = SessionLocal()
        try:
            if isinstance(transaction_id, int) or (isinstance(transaction_id, str) and transaction_id.isdigit()):
                transaction = session.query(NotarizeTransaction).filter_by(
            ...
            logger.info(f"Getting status for transaction {transaction.id}, external ID: {external_id}")
    ...
    def download_completed_document(
        ...
        try:
            ...
            logger.info(f"Downloading notarized document for transaction {transaction.id}")
            ...
            if doc_url:
                ...
                return {
                    'success': False,
                    'error': 'No document URL or ID found'
                }
            ...
            self._log_event(
                ...
                event_data={'external_id': external_id, 'document_index': document_index},
                client_id=transaction.client_id,
                request_id=external_id,
                response_status=200,
                db=session
            ...
            logger.info(f"Successfully downloaded document for transaction {transaction.id}")
    ...
    def handle_webhook(
        ...
        try:
            ...
            logger.info(f"Webhook processed for transaction {transaction.id}: {action_taken}")
