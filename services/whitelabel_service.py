...
from database import WhiteLabelConfig, FranchiseOrganization, FONT_FAMILIES
...
class WhiteLabelConfigService:
    ...
    def create_config(self, org_id: Optional[int], config_data: Dict[str, Any]) -> WhiteLabelConfig:
        """
        ...
        custom_domain = config_data.get('custom_domain', '').lower().strip() if config_data.get('custom_domain') else None
        
        if not subdomain:
            raise ValueError("Subdomain is required")
        
        if not self._validate_subdomain_format(subdomain):
            raise ValueError("Invalid subdomain format. Use only lowercase letters, numbers, and hyphens.")
        
        existing = self.db.query(WhiteLabelConfig).filter_by(subdomain=subdomain).first()
        if existing:
            raise ValueError(f"Subdomain '{subdomain}' is already in use")
        
        if custom_domain:
            existing = self.db.query(WhiteLabelConfig).filter_by(custom_domain=custom_domain).first()
            if existing:
        ...
        if config_data.get('email_from_address'):
            email_from_address_encrypted = encrypt_value(config_data['email_from_address'])
    ...
    def update_config(self, config_id: int, **kwargs) -> Optional[WhiteLabelConfig]:
        ...
        if 'email_from_address' in kwargs and kwargs['email_from_address']:
            kwargs['email_from_address_encrypted'] = encrypt_value(kwargs['email_from_address'])
        
        allowed_fields = [
            'organization_id', 'organization_name', 'subdomain', 'custom_domain',
            'logo_url', 'favicon_url', 'primary_color', 'secondary_color',
            'accent_color', 'header_bg_color', 'sidebar_bg_color', 'font_family',
            'custom_css', 'email_from_name', 'email_from_address', 'email_from_address_encrypted',
            'company_address', 'company_phone', 'company_email', 'footer_text',
            'terms_url', 'privacy_url', 'is_active'
        ]
    ...
    def validate_domain(self, domain: str, exclude_config_id: Optional[int] = None) -> Dict[str, Any]:
        """
...
"""
    ...
    def apply_branding(self, config: Optional[WhiteLabelConfig] = None) -> Dict[str, Any]:
        """
