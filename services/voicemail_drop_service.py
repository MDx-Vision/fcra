...
VOICEMAIL_CATEGORIES = [
    {"key": "welcome", "name": "Welcome", "description": "New client welcome messages"},
    {
        "key": "reminder",
        "name": "Reminder",
        "description": "Document and action reminders",
    },
...
class VoicemailDropService:
    ...
    def get_recording(self, recording_id: int) -> Optional[VoicemailRecording]:
        ...
        return (
            self.db.query(VoicemailRecording)
            .filter(VoicemailRecording.id == recording_id)
            .first()
        )

    def get_recordings(
        self, category: str = None, active_only: bool = True, limit: int = 100
    ...
    def _send_to_provider(
        self, drop: VoicemailDrop, recording: VoicemailRecording
    ) -> Dict[str, Any]:
    ...
    def get_drops(
        ...
        return (
            query.order_by(VoicemailDrop.created_at.desc())
            .offset(offset)
            .limit(limit)
            .all()
        )

    def get_client_drop_history(
    ...
    def retry_drop(self, drop_id: int) -> Dict[str, Any]:
        ...
        if drop.status not in ["failed", "cancelled"]:
            return {
                "success": False,
                "error": "Can only retry failed or cancelled drops",
            }
    ...
    def cancel_drop(self, drop_id: int) -> Dict[str, Any]:
        ...
        if drop.status not in ["pending", "scheduled"]:
            return {
                "success": False,
                "error": "Can only cancel pending or scheduled drops",
            }
    ...
    def get_campaign(self, campaign_id: int) -> Optional[VoicemailCampaign]:
        ...
        return (
            self.db.query(VoicemailCampaign)
            .filter(VoicemailCampaign.id == campaign_id)
            .first()
        )
    ...
    def add_targets_to_campaign(
        self, campaign_id: int, client_ids: List[int]
    ) -> Dict[str, Any]:
        ...
        if campaign.status != "draft":
            return {
                "success": False,
                "error": "Can only add targets to draft campaigns",
            }
    ...
    def cancel_campaign(self, campaign_id: int) -> Dict[str, Any]:
        ...
        self.db.query(VoicemailDrop).filter(
            VoicemailDrop.campaign_id == campaign_id, VoicemailDrop.status == "pending"
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        total_recordings = (
            self.db.query(func.count(VoicemailRecording.id))
            .filter(VoicemailRecording.is_active == True)
            .scalar()
        )
        ...
        drops_by_status = (
            self.db.query(VoicemailDrop.status, func.count(VoicemailDrop.id))
            .group_by(VoicemailDrop.status)
            .all()
        )
        ...
        total_cost = (
            self.db.query(func.sum(VoicemailDrop.cost_cents))
            .filter(VoicemailDrop.status.in_(["sent", "delivered"]))
            .scalar()
            or 0
        )

        active_campaigns = (
            self.db.query(func.count(VoicemailCampaign.id))
            .filter(VoicemailCampaign.status == "in_progress")
            .scalar()
        )
        ...
        recent_drops = (
            self.db.query(func.count(VoicemailDrop.id))
            .filter(VoicemailDrop.created_at >= yesterday)
            .scalar()
        )
    ...
    def get_recording_stats(self, recording_id: int) -> Dict[str, Any]:
        ...
        total_uses = (
            self.db.query(func.count(VoicemailDrop.id))
            .filter(VoicemailDrop.recording_id == recording_id)
            .scalar()
        )
