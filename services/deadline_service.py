...
def get_deadline_type_info(deadline_type):
    ...
    return DEADLINE_TYPES.get(deadline_type, {
...
def create_deadline(db, client_id, case_id, deadline_type, bureau, dispute_round, start_date, days_allowed=None):
    """
...
def get_upcoming_deadlines(db, days_ahead=7, include_overdue=True):
    ...
    try:
        ...
        logger.info(f"Retrieved {len(deadlines)} upcoming deadlines (next {days_ahead} days)")
        return deadlines
...
def extend_deadline(db, deadline_id, new_days, notes=None):
    ...
    try:
        ...
        if notes:
            existing_notes = deadline.notes or ''
            extension_note = f"[Extended] {old_deadline_date} -> {new_deadline_date}: {notes}"
...
def check_and_send_reminders(db):
    ...
    try:
        active_deadlines = db.query(CaseDeadline, Client).join(
            Client, CaseDeadline.client_id == Client.id
        ).filter(
            CaseDeadline.status == 'active'
        ).all()
        ...
        for deadline, client in active_deadlines:
            ...
            try:
                if days_remaining < 0 and not deadline.overdue_notice_sent:
                    if _send_deadline_reminder(db, deadline, client, 'overdue', days_remaining):
                    ...
                    if _send_deadline_reminder(db, deadline, client, '1_day', days_remaining):
                        deadline.reminder_sent_1_day = True
                        results['reminders_1_day'] += 1
                        logger.info(f"Sent 1-day reminder for deadline {deadline.id}")
                
                elif days_remaining <= 3 and days_remaining > 1 and not deadline.reminder_sent_3_days:
                    if _send_deadline_reminder(db, deadline, client, '3_days', days_remaining):
                        deadline.reminder_sent_3_days = True
                        results['reminders_3_days'] += 1
                        logger.info(f"Sent 3-day reminder for deadline {deadline.id}")
                
                elif days_remaining <= 7 and days_remaining > 3 and not deadline.reminder_sent_7_days:
                    if _send_deadline_reminder(db, deadline, client, '7_days', days_remaining):
        ...
        total_sent = (
            results['reminders_7_days'] + results['reminders_3_days'] + 
            results['reminders_1_day'] + results['overdue_notices']
        )
        logger.info(
...
def _send_deadline_reminder(db, deadline, client, reminder_type, days_remaining):
    ...
    if result['success']:
        logger.info(f"Sent {reminder_type} reminder to {client.email} for deadline {deadline.id}")
    else:
        logger.error(f"Failed to send {reminder_type} reminder to {client.email}: {result.get('error')}")
    
    return result['success']


def _build_deadline_email(first_name, deadline_type_name, bureau, deadline_date, 
                          days_remaining, reminder_type, dispute_round):
    ...
    if reminder_type == 'overdue':
        ...
        urgency_text = f"This deadline is due TOMORROW ({deadline_date.strftime('%B %d, %Y')})."
...
def get_deadline_summary(db, client_id=None):
    ...
    try:
        ...
        due_this_week = sum(1 for d in active if today <= d.deadline_date <= today + timedelta(days=7))
...
def create_cra_response_deadline(db, client_id, case_id, bureau, dispute_round, letter_sent_date):
    """
...
def create_reinvestigation_deadline(db, client_id, case_id, bureau, dispute_round, start_date):
    """
