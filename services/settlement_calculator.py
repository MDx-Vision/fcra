...
def calculate_settlement(case_id, client_id, violations_data):
    ...
    actual_damages, actual_damages_breakdown = _calculate_actual_damages(
        actual_damages_items
    )

    punitive_low, punitive_high = _calculate_punitive_damages(
...
def _save_settlement_estimate(
    ...
    willful_violations,
    negligent_violations,
    statutory_damages_low,
    statutory_damages_high,
    actual_damages,
    actual_damages_breakdown,
    punitive_damages_low,
    punitive_damages_high,
    attorney_fees_estimate,
    total_low,
    total_high,
    settlement_likelihood,
    recommended_demand,
...
def get_settlement_estimate(estimate_id):
    ...
    try:
        estimate = (
            ...
            .filter(SettlementEstimate.id == estimate_id)
            .first()
        )
        return estimate
    finally:
...
def get_client_settlement_estimates(client_id):
    ...
    try:
        estimates = (
            db.query(SettlementEstimate)
            .filter(SettlementEstimate.client_id == client_id)
