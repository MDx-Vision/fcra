...
from database import Client, CROAProgress, SessionLocal
...
class ScheduledJobsService:
    ...
    def capture_due_payments(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ready_clients = (
                ...
                .join(CROAProgress, Client.id == CROAProgress.client_id)
                .filter(
            ...
            self.db.rollback()
            return {"success": False, "job": "capture_due_payments", "error": str(e)}
    ...
    def expire_stale_holds(self, days_old: int = 7) -> Dict[str, Any]:
        ...
        """
        results = {"processed": 0, "expired": 0, "details": []}
        ...
        try:
            ...
            stale_clients = (
                self.db.query(Client)
                .filter(
                    Client.client_stage == "onboarding", Client.created_at < cutoff_date
                )
                .all()
            )
            ...
            for client in stale_clients:
                ...
                client.cancellation_reason = (
                    f"Onboarding not completed within {days_old} days"
                )
            ...
            self.db.rollback()
            return {"success": False, "job": "expire_stale_holds", "error": str(e)}
    ...
    def send_payment_reminders(self) -> Dict[str, Any]:
        ...
        Run this daily via cron.
        """
        results = {"processed": 0, "sent": 0, "details": []}
        ...
        try:
            ...
            for client in upcoming_clients:
                try:
                    ...
                    logger.error(
                        f"Failed to send payment reminder to {client.email}: {e}"
                    )
                    results["details"].append(
                        {
                            "client_id": client.id,
            ...
            failed_clients = (
                self.db.query(Client)
                .filter(Client.client_stage == "payment_failed")
                .all()
            )
            ...
            for client in failed_clients:
                try:
                    ...
                    send_payment_failed_email(
                        client_email=client.email, client_name=client_name
                    )
                    ...
                    logger.error(
                        f"Failed to send payment failed email to {client.email}: {e}"
                    )
                    results["details"].append(
                        {
                            "client_id": client.id,
            ...
            return {"success": False, "job": "send_payment_reminders", "error": str(e)}
    ...
    def get_pending_activations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for client in ready_clients:
                croa = (
                    self.db.query(CROAProgress).filter_by(client_id=client.id).first()
                )
                clients_list.append(
                    {
                        "client_id": client.id,
                        "name": client.name,
                        "email": client.email,
                        "cancellation_ended_at": (
                            croa.cancellation_period_ends_at.isoformat()
                            if croa
                            else None
                        ),
                        "ready_since": (
                            (now - croa.cancellation_period_ends_at).days
                            if croa
                            else None
                        ),
                    }
