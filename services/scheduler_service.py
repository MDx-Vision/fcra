...
from database import get_db, ScheduledJob
...
class SchedulerService:
        ...
        try:
            for schedule in SchedulerService.BUILT_IN_SCHEDULES:
                existing = session.query(ScheduledJob).filter(
                    ScheduledJob.name == schedule['name']
                ).first()
    ...
    def get_scheduler_stats() -> Dict[str, Any]:
        ...
        try:
            ...
            active = session.query(ScheduledJob).filter(ScheduledJob.is_active == True).count()
            paused = total - active
            
            from sqlalchemy import func
            total_runs = session.query(func.sum(ScheduledJob.run_count)).scalar() or 0
            
            next_job = session.query(ScheduledJob).filter(
                and_(
                    ScheduledJob.is_active == True,
                    ScheduledJob.next_run.isnot(None)
                )
            ).order_by(ScheduledJob.next_run.asc()).first()
...
COMMON_CRON_EXPRESSIONS = {
    ...
    'weekly_monday_8am': {'expression': '0 8 * * 1', 'label': 'Weekly on Monday at 8:00 AM'},
    'weekly_friday_5pm': {'expression': '0 17 * * 5', 'label': 'Weekly on Friday at 5:00 PM'},
    'monthly_first': {'expression': '0 0 1 * *', 'label': 'Monthly on the 1st at midnight'},
}
