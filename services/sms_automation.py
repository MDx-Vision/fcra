...
from .sms_service import send_sms, is_twilio_configured, format_phone_number
from .sms_templates import (
    welcome_sms, document_reminder_sms, case_update_sms,
    dispute_sent_sms, cra_response_sms, payment_reminder_sms,
    appointment_reminder_sms, round_started_sms, document_uploaded_sms,
    analysis_ready_sms, letters_ready_sms, custom_sms
)
...
def log_sms(db, client_id, phone_number, message, template_type, success, message_sid=None, error=None):
    """
...
def trigger_welcome_sms(db, client_id):
    ...
    log_sms(
        db, client_id, phone, message, 'welcome',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_document_reminder(db, client_id, doc_type="required documents"):
    ...
    log_sms(
        db, client_id, phone, message, 'document_reminder',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_case_update(db, client_id, new_status):
    ...
    log_sms(
        db, client_id, phone, message, 'case_update',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_dispute_sent(db, client_id, bureau):
    ...
    log_sms(
        db, client_id, phone, message, 'dispute_sent',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_cra_response(db, client_id, bureau):
    ...
    log_sms(
        db, client_id, phone, message, 'cra_response',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_payment_reminder(db, client_id, amount=None):
    ...
    log_sms(
        db, client_id, phone, message, 'payment_reminder',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_analysis_ready(db, client_id):
    ...
    log_sms(
        db, client_id, phone, message, 'analysis_ready',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_letters_ready(db, client_id, letter_count):
    ...
    log_sms(
        db, client_id, phone, message, 'letters_ready',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def trigger_round_started(db, client_id, round_number):
    ...
    log_sms(
        db, client_id, phone, message, 'round_started',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def send_custom_sms(db, client_id, message_text):
    ...
    log_sms(
        db, client_id, phone, message, 'custom',
        result['success'], result.get('message_sid'), result.get('error')
    )
...
def get_clients_needing_document_reminder(db, hours=24):
    ...
    clients_with_missing_docs = db.query(Client).filter(
