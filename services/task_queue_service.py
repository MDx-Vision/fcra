...
from database import get_db, BackgroundTask
...
def handle_send_email(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if template:
        success = email_service.send_template_email(to_email, subject, template, template_data)
    else:
...
def handle_send_sms(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {'success': result.get('success', False), 'to_phone': to_phone, 'sid': result.get('sid')}
...
def generate_case_summary_report(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        new_today = session.query(Client).filter(
...
def generate_revenue_report(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {'success': True, 'report_type': 'revenue', 'data': {'message': 'No payment data available'}}
    finally:
        session.close()
...
def generate_sol_deadline_report(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from database import get_db, NegativeItem
    ...
    try:
        ...
        items = session.query(NegativeItem).filter(
            and_(
        ...
        for item in items:
            upcoming_deadlines.append({
                ...
                'expiration_date': item.sol_expiration_date.isoformat() if item.sol_expiration_date else None
            })
        ...
        return {'success': True, 'report_type': 'sol_deadlines', 'data': {'message': 'SOL check completed', 'upcoming_count': 0}}
    finally:
        session.close()
...
def generate_dispute_letter(client_id: int, letter_type: str, bureau: str) -> Dict[str, Any]:
...
def handle_analyze_report(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        analysis = ai_service.analyze_credit_report(report_data) if hasattr(ai_service, 'analyze_credit_report') else None
...
class TaskQueueService:
    ...
    def _execute_task(session, task: BackgroundTask) -> Dict[str, Any]:
        ...
        try:
            ...
            if task.retries >= task.max_retries:
                ...
                task.status = 'pending'
                task.error_message = f"Retry {task.retries}/{task.max_retries}: {str(e)}"
    ...
    def get_tasks(
        ...
        try:
            ...
            if status:
                if status == 'active':
                    query = query.filter(BackgroundTask.status.in_(['pending', 'running']))
                else:
    ...
    def get_task_stats() -> Dict[str, Any]:
        ...
        try:
            ...
            ).group_by(BackgroundTask.status).all()
            ...
            completed_today = session.query(BackgroundTask).filter(
                and_(
