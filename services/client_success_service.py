...
from database import (
    Client, ClientSuccessMetric, DisputeItem, CRAResponse,
    CreditScoreSnapshot, Analysis, Violation, Case
)
...
class ClientSuccessService:
    ...
    def calculate_client_metrics(self, client_id: int) -> Dict[str, Any]:
        ...
        if initial_data.get('total_negatives', 0) > 0:
            deletion_rate = item_metrics['total_deleted'] / initial_data['total_negatives']
    ...
    def _get_initial_data(self, client_id: int) -> Dict[str, Any]:
        ...
        first_snapshot = self.db.query(CreditScoreSnapshot).filter(
            CreditScoreSnapshot.client_id == client_id
        ).order_by(CreditScoreSnapshot.created_at.asc()).first()

        if first_snapshot:
            avg_score = None
            scores = [s for s in [first_snapshot.equifax_score, first_snapshot.experian_score,
                                   first_snapshot.transunion_score] if s]
            if scores:
        ...
        initial_items = self.db.query(DisputeItem).filter(
            DisputeItem.client_id == client_id,
            DisputeItem.dispute_round == 1
        ).count()
    ...
    def _get_current_data(self, client_id: int) -> Dict[str, Any]:
        ...
        latest_snapshot = self.db.query(CreditScoreSnapshot).filter(
            CreditScoreSnapshot.client_id == client_id
        ).order_by(CreditScoreSnapshot.created_at.desc()).first()

        if latest_snapshot:
            avg_score = None
            scores = [s for s in [latest_snapshot.equifax_score, latest_snapshot.experian_score,
                                   latest_snapshot.transunion_score] if s]
            if scores:
                avg_score = sum(scores) // len(scores)
        ...
        remaining_items = self.db.query(DisputeItem).filter(
            DisputeItem.client_id == client_id,
    ...
    def _calculate_score_changes(self, initial: Dict, current: Dict) -> Dict[str, int]:
        ...
        if initial.get('transunion_score') and current.get('transunion_score'):
            changes['transunion'] = current['transunion_score'] - initial['transunion_score']
    ...
    def get_latest_snapshot(self, client_id: int) -> Optional[ClientSuccessMetric]:
        ...
        return self.db.query(ClientSuccessMetric).filter(
            ClientSuccessMetric.client_id == client_id
        ).order_by(ClientSuccessMetric.created_at.desc()).first()
    ...
    def get_snapshot_history(
        ...
        return self.db.query(ClientSuccessMetric).filter(
            ClientSuccessMetric.client_id == client_id
        ).order_by(ClientSuccessMetric.snapshot_date.desc()).limit(limit).all()
    ...
    def update_all_clients(self, snapshot_type: str = 'periodic') -> Dict[str, Any]:
        ...
        active_clients = self.db.query(Client).filter(
            Client.status.in_(['active', 'pending', 'signup'])
        ).all()
    ...
    def get_aggregate_report(self, period: str = 'all') -> Dict[str, Any]:
        ...
        return {
            ...
            'completion_rate': round((completed_cases / total_clients * 100) if total_clients > 0 else 0, 1),
    ...
    def get_top_performers(self, limit: int = 10, metric: str = 'deletion_rate') -> List[Dict]:
        ...
        ).group_by(ClientSuccessMetric.client_id).subquery()
        ...
        for snapshot in snapshots:
            client = self.db.query(Client).filter(Client.id == snapshot.client_id).first()
            if client:
                results.append({
                    'client_id': client.id,
                    'client_name': client.name,
    ...
    def get_success_trend(self, period: str = 'month', granularity: str = 'week') -> List[Dict]:
        ...
        snapshots = self.db.query(ClientSuccessMetric).filter(
            ClientSuccessMetric.snapshot_date >= start_date,
            ClientSuccessMetric.snapshot_date <= end_date
        ).order_by(ClientSuccessMetric.snapshot_date).all()
        ...
        for snapshot in snapshots:
            ...
            if snapshot.deletion_rate:
                date_groups[date_key]['avg_deletion_rate'].append(snapshot.deletion_rate)
            if snapshot.avg_score_change:
                date_groups[date_key]['avg_score_change'].append(snapshot.avg_score_change)
        ...
        for date_key in sorted(date_groups.keys()):
            ...
            avg_del_rate = sum(group['avg_deletion_rate']) / len(group['avg_deletion_rate']) if group['avg_deletion_rate'] else 0
            avg_score = sum(group['avg_score_change']) / len(group['avg_score_change']) if group['avg_score_change'] else 0
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        recent_completions = self.db.query(Client).filter(
    ...
    def get_bureau_breakdown(self) -> Dict[str, Any]:
        ...
        snapshots = self.db.query(ClientSuccessMetric).join(
            subquery,
            and_(
                ClientSuccessMetric.client_id == subquery.c.client_id,
                ClientSuccessMetric.id == subquery.c.max_id
        ...
        for snapshot in snapshots:
            ...
            if any([snapshot.equifax_score_change, snapshot.experian_score_change, snapshot.transunion_score_change]):
                score_count += 1
        ...
        return {
            ...
            'avg_score_improvement': {
                'equifax': round(equifax_score_total / score_count, 1) if score_count else 0,
                'experian': round(experian_score_total / score_count, 1) if score_count else 0,
                'transunion': round(transunion_score_total / score_count, 1) if score_count else 0,
            },
