...
from database import ClientBadge, BadgeDefinition, Client, get_db
...
def seed_badge_definitions() -> Dict[str, Any]:
    ...
    for badge in DEFAULT_BADGES:
        existing = db.query(BadgeDefinition).filter(
            BadgeDefinition.badge_type == badge['badge_type']
        ).first()
...
def award_badge(client_id: int, badge_type: str, achievement_value: int = None) -> Dict[str, Any]:
    """Award a badge to a client"""
    db = get_db()

    # Check if badge type exists
    definition = db.query(BadgeDefinition).filter(
        BadgeDefinition.badge_type == badge_type,
    ...
    existing = db.query(ClientBadge).filter(
        ClientBadge.client_id == client_id,
        ClientBadge.badge_type == badge_type
    ).first()

    if existing:
        return {'success': False, 'error': 'Client already has this badge', 'already_awarded': True}
...
def get_client_badges(client_id: int) -> List[Dict[str, Any]]:
    ...
    db = get_db()
    badges = db.query(ClientBadge).filter(
        ClientBadge.client_id == client_id
    ).order_by(ClientBadge.earned_at.desc()).all()
...
def mark_badges_notified(client_id: int, badge_types: List[str] = None) -> Dict[str, Any]:
...
def check_and_award_badges(client_id: int, event_type: str, value: int = None) -> List[Dict[str, Any]]:
...
def get_badge_leaderboard(limit: int = 10) -> List[Dict[str, Any]]:
    ...
    results = db.query(
        ClientBadge.client_id,
        func.count(ClientBadge.id).label('badge_count')
    ).group_by(ClientBadge.client_id).order_by(
        func.count(ClientBadge.id).desc()
    ).limit(limit).all()
...
def get_badge_stats() -> Dict[str, Any]:
    ...
    clients_with_badges = db.query(func.count(func.distinct(ClientBadge.client_id))).scalar()

    # Most common badges
    popular = db.query(
        ClientBadge.badge_type,
        func.count(ClientBadge.id).label('count')
    ).group_by(ClientBadge.badge_type).order_by(
        func.count(ClientBadge.id).desc()
    ).limit(5).all()
...
def get_next_badges(client_id: int) -> List[Dict[str, Any]]:
    ...
    db = get_db()
    earned_types = [b.badge_type for b in db.query(ClientBadge).filter(
        ClientBadge.client_id == client_id
    ).all()]
    ...
    ).order_by(BadgeDefinition.trigger_threshold).limit(3).all()
...
def revoke_badge(client_id: int, badge_type: str) -> Dict[str, Any]:
    ...
    db = get_db()
    badge = db.query(ClientBadge).filter(
        ClientBadge.client_id == client_id,
        ClientBadge.badge_type == badge_type
    ).first()
