...
class AttorneyAnalyticsService:
    ...
    PERIOD_MAPPINGS = {"week": 7, "month": 30, "quarter": 90, "year": 365}
    ...
    def calculate_performance(
        self, staff_user_id: int, period: str = "month"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            total_settlements = sum(
                o.settlement_amount or 0
                for o in outcomes
                if o.final_outcome == "settled"
            )

            resolution_times = [
                o.time_to_resolution_days
                for o in outcomes
                if o.time_to_resolution_days and o.time_to_resolution_days > 0
            ]
            avg_resolution_days = (
            ...
            avg_settlement = (
                total_settlements / cases_settled if cases_settled > 0 else 0
            )

            win_rate = (
                (cases_won + cases_settled) / cases_handled if cases_handled > 0 else 0
            )
    ...
    def get_leaderboard(
        self, metric: str = "efficiency_score", period: str = "month"
    ) -> Dict[str, Any]:
        """
            ...
            for attorney in attorneys:
                ...
                if perf:
                    ...
                    if metric == "cases_handled":
                        ...
                        metric_value = sum(
                            1 for o in outcomes if o.final_outcome == "won"
                        )
                    elif metric == "cases_settled":
                        metric_value = sum(
                            1 for o in outcomes if o.final_outcome == "settled"
                        )
                        ...
                        times = [
                            o.time_to_resolution_days
                            for o in outcomes
                            if o.time_to_resolution_days
                        ]
                        metric_value = statistics.mean(times) if times else 0
                    else:
                        cases = len(outcomes)
                        wins = sum(
                            1 for o in outcomes if o.final_outcome in ["won", "settled"]
                        )
    ...
    def identify_strengths(self, staff_user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            outcomes = (
                db.query(CaseOutcome)
                .filter(CaseOutcome.attorney_id == staff_user_id)
                .all()
            )
            ...
            for o in outcomes:
                if o.violation_types:
                    for vtype in (
                        o.violation_types if isinstance(o.violation_types, list) else []
                    ):
            ...
            return {
                ...
                "overall_win_rate": round(
                    (
                        (settled + sum(1 for o in outcomes if o.final_outcome == "won"))
                        / total
                        * 100
                        if total > 0
                        else 0
    ...
    def recommend_case_assignment(self, case_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            attorneys = (
                db.query(Staff)
                .filter(Staff.role.in_(["attorney", "admin"]), Staff.is_active == True)
                .all()
            )
            ...
            for attorney in attorneys:
                ...
                outcomes = (
                    db.query(CaseOutcome)
                    .filter(CaseOutcome.attorney_id == attorney.id)
                    .all()
                )
                ...
                for vtype in violation_types:
                    matching = [
                        o
                        for o in outcomes
                        if o.violation_types and vtype in str(o.violation_types)
                    ]
                    if len(matching) >= 2:
                        score += 10
                ...
                if is_high_value:
                    high_value_wins = sum(
                        1
                        for o in outcomes
                        if (o.settlement_amount or 0) > 10000
                        and o.final_outcome == "settled"
                    )
                ...
                recommendations.append(
                    {
                        ...
                        "availability": (
                            "high"
                            if current_cases < 5
                            else ("medium" if current_cases < 10 else "low")
                        ),
                    }
                )
    ...
    def get_workload_distribution(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for attorney in attorneys:
                ...
                pending_settlements = (
                    db.query(Settlement)
                    .join(Case, Settlement.case_id == Case.id)
                ...
                distribution.append(
                    {
                        ...
                        "capacity_status": (
                            "overloaded"
                            if recent_cases > 15
                            else ("optimal" if recent_cases >= 5 else "available")
                        ),
                    }
                )
            ...
            overloaded = sum(
                1 for d in distribution if d["capacity_status"] == "overloaded"
            )
            available = sum(
                1 for d in distribution if d["capacity_status"] == "available"
            )
            ...
            return {
                ...
                "balance_recommendation": (
                    "Redistribute cases"
                    if overloaded > 0 and available > 0
    ...
    def forecast_capacity(self, staff_user_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            resolution_times = (
                db.query(CaseOutcome.time_to_resolution_days)
                .filter(
                    CaseOutcome.attorney_id == staff_user_id,
            ...
            avg_resolution = (
                statistics.mean([r[0] for r in resolution_times])
                if resolution_times
                else 45
            )

            cases_closing_soon = (
                int(current_cases * (30 / avg_resolution)) if avg_resolution > 0 else 0
            )
            ...
            for week in range(1, 5):
                ...
                expected_new = 2
                projected_load = max(
                    0, current_load - expected_completions + expected_new
                )
    ...
    def _calculate_efficiency_score(
        self, cases: int, win_rate: float, avg_days: float, total_value: float
    ) -> float:
    ...
    def _identify_strengths(self, outcomes: List) -> List[str]:
        ...
        times = [
            o.time_to_resolution_days for o in outcomes if o.time_to_resolution_days
        ]
        ...
        willful_cases = sum(
            1 for o in outcomes if o.willfulness_score and o.willfulness_score > 0.7
        )
        if willful_cases >= 2:
