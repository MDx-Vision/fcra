...
class AuditService:
    ...
    def _get_request_context(self) -> Dict[str, Any]:
        ...
        try:
            if request:
                ...
                context["request_id"] = (
                    getattr(g, "request_id", None) or str(uuid.uuid4())[:8]
                )
        except RuntimeError:
            pass
    ...
    def log_event(
        ...
        try:
            ...
            is_phi, phi_fields = self._detect_phi_access(
    ...
    def log_login(
        ...
        details = {"success": success, "login_method": "password"}
    ...
    def log_data_access(
        ...
        is_phi = any(
            field.lower() in [f.lower() for f in PHI_FIELDS]
            for field in (fields_accessed or [])
        )

        details = {"fields_accessed": fields_accessed, "access_reason": reason}
    ...
    def generate_compliance_report(
        ...
        try:
            ...
            report["summary"]["events_by_type"] = {
                event: count for event, count in event_counts
            }
            ...
            user_type_counts = (
                db.query(AuditLog.user_type, func.count(AuditLog.id))
                .filter(
                    AuditLog.timestamp >= start_date, AuditLog.timestamp <= end_date
                )
                .group_by(AuditLog.user_type)
                .all()
            )

            report["summary"]["events_by_user_type"] = {
                ut: count for ut, count in user_type_counts
            }

            if report_type == "soc2":
                login_failures = base_query.filter(
                    AuditLog.event_type == "login_failed"
                ).count()
                report["details"]["failed_logins"] = login_failures

                permission_changes = base_query.filter(
                    AuditLog.event_type == "permission_change"
                ).count()
                report["details"]["permission_changes"] = permission_changes

                config_changes = base_query.filter(
                    AuditLog.event_type == "settings_change"
                ).count()
                ...
                critical_events = (
                    base_query.filter(AuditLog.severity == "critical")
                    .order_by(desc(AuditLog.timestamp))
                    .limit(100)
                    .all()
                )
                report["details"]["critical_events"] = [
                    e.to_dict() for e in critical_events
                ]
                ...
                phi_by_user = (
                    ...
                    .group_by(AuditLog.user_id, AuditLog.user_email, AuditLog.user_name)
                    .all()
                )
                ...
                client_data_exports = base_query.filter(
                    AuditLog.event_type == "export", AuditLog.resource_type == "client"
                ).count()
                report["details"]["client_data_exports"] = client_data_exports

                recent_phi_logs = (
                    base_query.filter(AuditLog.is_phi_access == True)
                    .order_by(desc(AuditLog.timestamp))
                    .limit(100)
                    .all()
                )
                report["details"]["recent_phi_access"] = [
                    e.to_dict() for e in recent_phi_logs
                ]
    ...
    def get_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            stats["total_events"] = (
                db.query(AuditLog).filter(AuditLog.timestamp >= start_date).count()
            )

            daily_counts = (
                db.query(
                    func.date(AuditLog.timestamp).label("date"), func.count(AuditLog.id)
                )
            ...
            top_users = (
                ...
                .filter(AuditLog.timestamp >= start_date, AuditLog.user_id.isnot(None))
                .group_by(
                    AuditLog.user_id,
                    AuditLog.user_email,
                    AuditLog.user_name,
