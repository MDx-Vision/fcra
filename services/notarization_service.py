...
from database import NotarizationOrder, SessionLocal
...
def create_notarization_order(
    ...
    try:
        ...
        domain = os.environ.get("REPL_SLUG_URL") or os.environ.get(
            "REPLIT_DEV_DOMAIN", ""
        )
        if domain:
        ...
        logger.info(
            f"Creating notarization order for client {client_id}, document: {document_name}"
        )
        ...
        session_link = (
            response_data.get("session_link")
        ...
        logger.info(
            f"Created notarization order {order.id} with transaction ID {transaction_id}"
        )
...
def get_notarization_status(order_id):
    ...
    try:
        order = (
            session.query(NotarizationOrder)
            .filter(NotarizationOrder.id == order_id)
            .first()
        )
        ...
        logger.info(
            f"Getting status for notarization order {order_id}, transaction: {order.external_transaction_id}"
        )
        ...
        logger.info(
            f"Updated notarization order {order_id} status to: {normalized_status}"
        )
...
def handle_webhook(webhook_data):
    ...
    try:
        ...
        order = (
            session.query(NotarizationOrder)
            .filter(NotarizationOrder.external_transaction_id == transaction_id)
            .first()
        )
