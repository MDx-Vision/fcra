    ...
    def get_stats(self) -> Dict:
        ...
        with self._lock:
            ...
            for key, entry in self._store.items():
                is_expired = (
                    entry["expires_at"] and datetime.utcnow() > entry["expires_at"]
                )
                entries.append(
                    {
                        "key": key,
            ...
            return {
                ...
                "memory_estimate_kb": sum(
                    len(str(e["value"])) for e in self._store.values()
                )
                / 1024,
            }
...
class PerformanceService:
    ...
    def get_slow_endpoints(self, threshold_ms: float = 100) -> List[Dict]:
        ...
        with _metrics_lock:
                ...
                if stats["avg_response_time_ms"] > threshold_ms:
                    ...
                    if stats["cache_hit_rate"] < 50:
                        recommendations.append(
                            "Consider adding caching to this endpoint"
                        )
                    if stats["p99_time"] > stats["avg_response_time_ms"] * 3:
                        recommendations.append(
                            "High p99 variance - check for occasional slow queries"
                        )
                    if stats["error_rate"] > 5:
                        recommendations.append(
                            "High error rate - investigate error causes"
                        )
                    if stats["request_count"] > 100:
                        recommendations.append(
                            "High traffic endpoint - prioritize optimization"
                        )
            ...
            return sorted(
                slow_endpoints, key=lambda x: x["avg_response_time_ms"], reverse=True
            )
    ...
    def get_performance_summary(self, period_minutes: int = 60) -> Dict:
        ...
        with _metrics_lock:
            ...
            return {
                ...
                "requests_per_minute": (
                    round(total_requests / period_minutes, 2)
                    if period_minutes > 0
                    else 0
                ),
                "avg_response_time_ms": (
                    round(statistics.mean(all_durations), 2) if all_durations else 0
                ),
                ...
                "cache_hit_rate": (
                    round(total_cache_hits / total_requests * 100, 2)
                    if total_requests > 0
                    else 0
                ),
                ...
                "most_active_endpoints": sorted(
                    endpoint_stats, key=lambda x: x["request_count"], reverse=True
                )[:5],
                "highest_error_endpoints": sorted(
                    endpoint_stats, key=lambda x: x["error_rate"], reverse=True
                )[:5],
            }
    ...
    def optimize_query(self, query_string: str) -> Dict:
        ...
        if query_lower.count(" join ") > 3:
            ...
            recommendations.append(
                "Consider breaking into subqueries or using temp tables"
            )
        ...
        if "in (" in query_lower:
            ...
            for in_clause in in_values:
                if in_clause.count(",") > 100:
                    ...
                    recommendations.append(
                        "Consider using a temporary table or subquery"
                    )
        ...
        try:
            ...
            if self.db:
                try:
                    ...
                    if result:
                        db_stats["database_size_mb"] = round(
                            result[0] / (1024 * 1024), 2
                        )
                except:
                    pass
        ...
        common_index_patterns = [
            {
                "table": "clients",
                "column": "email",
                "reason": "Frequently queried for lookups",
            },
            ...
            },
            {
                "table": "audit_logs",
                "column": "timestamp",
                "reason": "Time-range queries common",
            },
            {
                "table": "cases",
                "column": "attorney_id",
                "reason": "Joined/filtered by attorney",
            },
            {
                "table": "cases",
                "column": "status",
                "reason": "Pipeline stage filtering",
            },
            {
                "table": "analyses",
                "column": "client_id",
                "reason": "Client lookup common",
            },
            {
        ...
        with _metrics_lock:
            for key in list(_request_metrics.keys()):
                ...
                _request_metrics[key] = [
                    m for m in _request_metrics[key] if m["timestamp"] > cutoff
                ]
...
def request_timing_middleware(app):
    ...
    def after_request(response):
        if hasattr(g, "request_start_time"):
            ...
            if duration_ms > 100:
                print(
                    f"⚠️  SLOW REQUEST: {request.method} {request.path} took {duration_ms:.2f}ms"
                )
