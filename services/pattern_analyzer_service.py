...
from database import SessionLocal, FurnisherPattern, CaseOutcome, Furnisher, FurnisherStats, Violation, Staff, Client
import json
import math
...
class PatternAnalyzerService:
    ...
    def _get_cache(self, key):
        if key in self._cache and datetime.utcnow() < self._cache_expiry.get(key, datetime.min):
    ...
    def analyze_furnisher_behavior(self, furnisher_id: int = None, furnisher_name: str = None) -> dict:
        """
        ...
        try:
            ...
            if furnisher_id:
                ...
                furnisher = db.query(Furnisher).filter(Furnisher.id == furnisher_id).first()
                fname = furnisher.name if furnisher else f"Furnisher #{furnisher_id}"
            elif furnisher_name:
                furnisher = db.query(Furnisher).filter(Furnisher.name.ilike(f'%{furnisher_name}%')).first()
                if furnisher:
            ...
            settlement_amounts = [o.settlement_amount for o in outcomes if o.settlement_amount > 0]
            avg_settlement = sum(settlement_amounts) / len(settlement_amounts) if settlement_amounts else 0
            
            resolution_times = [o.time_to_resolution_days for o in outcomes if o.time_to_resolution_days > 0]
            ...
            if settlement_rate >= 0.7:
                ...
                insights.append(f"High settlement rate ({round(settlement_rate * 100)}%). Consider aggressive initial demands.")
                ...
                insights.append("Low settlement rate. Prepare comprehensive litigation documentation.")
            ...
            if avg_resolution > 0:
                if avg_resolution < 45:
                    ...
                    insights.append(f"Fast resolution time ({round(avg_resolution)} days avg). Quick action recommended.")
                    ...
                    insights.append(f"Slow resolution ({round(avg_resolution)} days avg). Prepare for extended timeline.")
            ...
            if best_violation:
                ...
                insights.append(f"Focus on {best_violation} violations - highest success rate ({round(best_rate * 100)}%).")
            
            self._save_pattern(db, furnisher_id, fname, 'response_behavior', {
            ...
            return {
                ...
                'recommendation': insights[0] if insights else 'Continue gathering data for this furnisher.'
            }
    ...
    def _save_pattern(self, db, furnisher_id, furnisher_name, pattern_type, pattern_data, 
                      sample_size, confidence):
        """Save or update a pattern record"""
        try:
            existing = db.query(FurnisherPattern).filter(
    ...
    def detect_seasonal_trends(self) -> dict:
        ...
        try:
            ...
            for outcome in outcomes:
                if outcome.created_at:
                    ...
                    if outcome.settlement_amount > 0:
                        monthly_data[month]['settlements'].append(outcome.settlement_amount)
                    if outcome.time_to_resolution_days > 0:
                        monthly_data[month]['resolution_days'].append(outcome.time_to_resolution_days)
            ...
            for month, data in monthly_data.items():
                if data['total'] > 0:
                    monthly_stats[month] = {
                        ...
                        'avg_settlement': round(sum(data['settlements']) / len(data['settlements']), 2) if data['settlements'] else 0,
                        'avg_resolution': round(sum(data['resolution_days']) / len(data['resolution_days'])) if data['resolution_days'] else 0
                    }
            ...
            best_month = max(monthly_stats.keys(), key=lambda m: monthly_stats[m]['success_rate'], default=None)
            worst_month = min(monthly_stats.keys(), key=lambda m: monthly_stats[m]['success_rate'], default=None)
            ...
            if best_month and monthly_stats[best_month]['success_rate'] > 0.6:
                ...
                recommendations.append(f"Consider timing filings around {month_names[best_month]} for better outcomes.")
            ...
            if worst_month and monthly_stats[worst_month]['success_rate'] < 0.4:
                ...
                recommendations.append(f"Avoid major filings in {month_names[worst_month]} if possible.")
    ...
    def identify_winning_strategies(self, violation_type: str = None) -> dict:
        ...
        try:
            ...
            unsuccessful = [o for o in outcomes if o.final_outcome in ['lost', 'dismissed']]
            ...
            for outcome in outcomes:
                for vtype in (outcome.violation_types or []):
                    if vtype not in success_by_violation:
                        success_by_violation[vtype] = {'success': 0, 'total': 0, 'settlements': []}
                    success_by_violation[vtype]['total'] += 1
                    if outcome.final_outcome in ['won', 'settled']:
                        success_by_violation[vtype]['success'] += 1
                        if outcome.settlement_amount > 0:
                            success_by_violation[vtype]['settlements'].append(outcome.settlement_amount)
            
            for vtype, data in success_by_violation.items():
                if data['total'] >= 3:
                    rate = data['success'] / data['total']
                    avg_settlement = sum(data['settlements']) / len(data['settlements']) if data['settlements'] else 0
                    strategies.append({
            ...
            if high_doc:
                high_doc_settlements = [o.settlement_amount for o in high_doc if o.settlement_amount > 0]
                if high_doc_settlements:
                    strategies.append({
                        'type': 'documentation_impact',
                        'finding': 'Strong documentation correlates with higher settlements',
                        'avg_settlement_high_doc': round(sum(high_doc_settlements) / len(high_doc_settlements), 2),
            ...
            if with_attorney and without_attorney:
                with_rate = sum(1 for o in with_attorney if o.final_outcome in ['won', 'settled']) / len(with_attorney)
                without_rate = sum(1 for o in without_attorney if o.final_outcome in ['won', 'settled']) / len(without_attorney)
            ...
            if rounds_data:
                best_round = max(rounds_data.keys(), 
                               key=lambda r: sum(rounds_data[r]['settlements']) / max(len(rounds_data[r]['settlements']), 1) if rounds_data[r]['settlements'] else 0)
                if rounds_data[best_round]['settlements']:
                    strategies.append({
                        'type': 'optimal_rounds',
                        'best_round': best_round,
                        'avg_settlement': round(sum(rounds_data[best_round]['settlements']) / len(rounds_data[best_round]['settlements']), 2),
            ...
            return {
                ...
                'top_violation_types': [s for s in strategies if s['type'] == 'violation_effectiveness'][:5],
    ...
    def find_attorney_strengths(self) -> dict:
        ...
        try:
            outcomes = db.query(CaseOutcome).filter(
                CaseOutcome.attorney_id.isnot(None)
            ).all()
            ...
            for aid, data in attorney_data.items():
                ...
                attorneys.append({
                    ...
                    'specialization_success_rate': round(best_rate, 3) if best_violation else None,
                    'strengths': self._identify_attorney_strengths(data, success_rate, avg_settlement)
                })
    ...
    def _generate_attorney_insights(self, attorneys):
        ...
        if len(attorneys) >= 2:
            top = attorneys[0]
            insights.append(f"{top['name']} has the highest success rate at {round(top['success_rate'] * 100)}%")
        
        high_settlers = [a for a in attorneys if a['avg_settlement'] >= 5000]
        if high_settlers:
            best_settler = max(high_settlers, key=lambda x: x['avg_settlement'])
            insights.append(f"{best_settler['name']} achieves highest average settlements (${best_settler['avg_settlement']:,.2f})")
        
        specialists = [a for a in attorneys if a.get('specialization')]
        for spec in specialists[:2]:
            insights.append(f"{spec['name']} specializes in {spec['specialization']} cases ({round(spec['specialization_success_rate'] * 100)}% success)")
    ...
    def get_pattern_insights(self, filters: dict = None) -> dict:
        ...
        db = SessionLocal()
        try:
            patterns = db.query(FurnisherPattern).order_by(
                FurnisherPattern.confidence.desc()
            ).limit(50).all()
            
            filters = filters or {}
            
            if filters.get('furnisher_id'):
                patterns = [p for p in patterns if p.furnisher_id == filters['furnisher_id']]
            
            if filters.get('pattern_type'):
                patterns = [p for p in patterns if p.pattern_type == filters['pattern_type']]
...
def analyze_furnisher_behavior(furnisher_id: int = None, furnisher_name: str = None) -> dict:
