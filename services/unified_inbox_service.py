...
ALL_CHANNELS = [CHANNEL_EMAIL, CHANNEL_SMS, CHANNEL_WHATSAPP, CHANNEL_PORTAL, CHANNEL_CHAT]
...
class UnifiedInboxService:
    ...
    def _whatsapp_to_unified(self, wa_msg, client=None) -> UnifiedMessage:
        ...
        return UnifiedMessage(
            ...
            preview=wa_msg.body or 'Media message' if wa_msg.has_media else 'No message',
            sender_name=wa_msg.profile_name if direction == DIRECTION_INBOUND else 'System',
            recipient=wa_msg.to_number if direction == DIRECTION_OUTBOUND else wa_msg.from_number,
    ...
    def _portal_message_to_unified(self, msg, client=None, staff=None) -> UnifiedMessage:
    ...
    def _chat_message_to_unified(self, chat_msg, conversation=None, client=None) -> UnifiedMessage:
    ...
    def get_client_inbox(
        ...
        from database import (
            EmailLog, SMSLog, WhatsAppMessage, ClientMessage,
            ChatMessage, ChatConversation, Client, Staff
        )
        ...
        if CHANNEL_WHATSAPP in channels:
            query = db.query(WhatsAppMessage).filter(WhatsAppMessage.client_id == client_id)
            if search_query:
        ...
        if CHANNEL_PORTAL in channels:
            ...
            for msg in query.all():
                staff = db.query(Staff).filter(Staff.id == msg.staff_id).first() if msg.staff_id else None
        ...
        if CHANNEL_CHAT in channels:
            ...
            for conv in conversations:
                ...
                if search_query:
                    chat_query = chat_query.filter(ChatMessage.content.ilike(f'%{search_query}%'))
                if direction == DIRECTION_INBOUND:
                ...
                for chat_msg in chat_query.all():
                    messages.append(self._chat_message_to_unified(chat_msg, conv, client))
    ...
    def mark_client_messages_read(
        ...
        if channel is None or channel == CHANNEL_PORTAL:
            query = db.query(ClientMessage).filter(
                ...
                ClientMessage.sender_type == 'client'  # Only mark client messages as read
            )
    ...
    def get_conversation_thread(
        ...
        include_channels = include_channels or [CHANNEL_PORTAL, CHANNEL_CHAT, CHANNEL_SMS]
