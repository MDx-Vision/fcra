...
def extract_text_from_pdf(file_path: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not text or len(text.strip()) < 100:
        return None, "This appears to be an image-based PDF. Please use the OCR scanner instead."
...
def extract_accounts(text: str) -> List[Dict[str, Any]]:
    ...
    sections = re.split(r'(?:REVOLVING|INSTALLMENT|MORTGAGE|ACCOUNTS?|TRADELINE)', text, flags=re.IGNORECASE)
    ...
    for section in sections:
        ...
        for match in creditor_pattern.finditer(section):
            ...
            if creditor and len(creditor) >= 3:
                ...
                balance_match = re.search(r"(?:Balance|Current Balance|High Balance)[:\s]*\$?\s*([\d,]+(?:\.\d{2})?)", context, re.IGNORECASE)
                balance = extract_currency(balance_match.group(1)) if balance_match else None
                
                status_match = re.search(r"(?:Status|Account Status|Condition)[:\s]*([A-Za-z\s]+?)(?:\n|$|,)", context, re.IGNORECASE)
                status = classify_account_status(status_match.group(1)) if status_match else classify_account_status(context)
                
                opened_match = re.search(r"(?:Date Opened|Opened|Open Date)[:\s]*([\d/\-]+)", context, re.IGNORECASE)
                opened = normalize_date(opened_match.group(1)) if opened_match else None
                
                limit_match = re.search(r"(?:Credit Limit|Limit)[:\s]*\$?\s*([\d,]+(?:\.\d{2})?)", context, re.IGNORECASE)
                limit = extract_currency(limit_match.group(1)) if limit_match else None
                
                payment_history = []
                history_match = re.search(r"(?:Payment History|Pay Status)[:\s]*([OCXR123456789]{1,24})", context, re.IGNORECASE)
                if history_match:
                ...
                if not any(a.get("name", "").upper() == creditor.upper() and a.get("number") == account_num for a in accounts):
    ...
    if len(accounts) < 3:
        ...
        for match in generic_pattern.finditer(text):
            ...
            if creditor and len(creditor) >= 3:
                if not any(a.get("name", "").upper() == creditor.upper() for a in accounts):
                    accounts.append({
                        "name": creditor,
                        "number": account_num,
                        "balance": balance,
...
def _detect_account_type(creditor: str, context: str) -> str:
    ...
    if any(kw in text for kw in ["mortgage", "home loan", "real estate"]):
        ...
        return "Student Loan"
    elif any(kw in text for kw in ["credit card", "card", "visa", "mastercard", "amex", "discover"]):
        return "Credit Card"
...
def extract_inquiries(text: str) -> List[Dict[str, Any]]:
    ...
    inquiry_markers = ["INQUIRIES", "CREDIT INQUIRIES", "REGULAR INQUIRIES", "HARD INQUIRIES", "REQUESTS FOR YOUR CREDIT"]
    
    for marker in inquiry_markers:
        idx = text.upper().find(marker)
        if idx != -1:
            end_markers = ["PUBLIC RECORD", "COLLECTION", "ACCOUNT SUMMARY", "SCORE", "END OF REPORT"]
            end_idx = len(text)
    ...
    for match in inquiry_pattern.finditer(inquiry_section):
        ...
        if company and len(company) >= 3:
            ...
            if not any(word in company.upper() for word in skip_words):
                ...
                if not any(i.get("company", "").upper() == company.upper() and i.get("date") == date for i in inquiries):
...
def extract_collections(text: str) -> List[Dict[str, Any]]:
    ...
    collection_markers = ["COLLECTION", "COLLECTIONS", "COLLECTION ACCOUNTS", "ACCOUNTS IN COLLECTION"]
    ...
    for match in collection_pattern.finditer(collection_section):
        ...
        if agency and len(agency) >= 3:
            collection_keywords = ["collection", "agency", "recovery", "associates", "services", "portfolio", "midland", "lvnv", "cavalry", "encore"]
            ...
            if is_collection or (amount and amount > 0):
                ...
                if not any(c.get("agency", "").upper() == agency.upper() for c in collections):
...
def extract_public_records(text: str) -> List[Dict[str, Any]]:
    ...
    public_section = None
    public_markers = ["PUBLIC RECORD", "PUBLIC RECORDS", "BANKRUPTCIES", "JUDGMENTS", "TAX LIENS"]
    
    for marker in public_markers:
        idx = text.upper().find(marker)
        if idx != -1:
            end_markers = ["COLLECTION", "INQUIRIES", "ACCOUNT SUMMARY", "TRADELINE", "ACCOUNTS"]
    ...
    for record_type, keywords in record_types.items():
        for keyword in keywords:
            ...
            for match in pattern.finditer(public_section):
                ...
                if not any(r.get("type") == record_type and r.get("date_filed") == date for r in records):
...
def parse_credit_report_pdf(file_path: str) -> Dict[str, Any]:
    ...
    if not text or len(text.strip()) < 100:
        result["error"] = "Could not extract sufficient text from PDF. This may be an image-based PDF - please use the OCR scanner instead."
        return result
    ...
    logger.info(f"Parsed credit report: bureau={result['bureau']}, accounts={len(result['accounts'])}, confidence={result['parsing_confidence']:.2f}")
...
def get_parsed_text_for_analysis(parsed_result: Dict[str, Any]) -> str:
    ...
    if inquiries:
        ...
        for inq in inquiries:
            lines.append(f"  {inq.get('date', 'Unknown date')} - {inq.get('company', 'Unknown')}")
        lines.append("")
    ...
    if public_records:
        ...
        for rec in public_records:
            ...
            lines.append(f"  {rec.get('type', 'Unknown')} ({rec.get('date_filed', 'Unknown date')}){amount_str}")
        lines.append("")
