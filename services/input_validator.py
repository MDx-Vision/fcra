...
SQL_INJECTION_PATTERNS = [
    re.compile(
        r"(\b(SELECT|INSERT|UPDATE|DELETE|DROP|UNION|ALTER|CREATE|TRUNCATE)\b)", re.I
    ),
    re.compile(r"(--|#|/\*|\*/)", re.I),
...
def sanitize_credit_report_html(html_content: Optional[str]) -> Optional[str]:
    ...
    html_content = re.sub(
        r"<script[^>]*>.*?</script>", "", html_content, flags=re.I | re.S
    )

    # Remove event handlers
    html_content = re.sub(
        r'\s+on\w+\s*=\s*["\'][^"\']*["\']', "", html_content, flags=re.I
    )
