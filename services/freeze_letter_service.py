...
from database import Client, FreezeLetterBatch, get_db
...
def generate_single_freeze_letter(client, bureau_name, bureau_address):
    ...
    client_full_address = (
        f"{address_street}, {address_city}, {address_state} {address_zip}".strip(", ")
    )
...
def _create_freeze_letter_docx(letter_content):
    ...
    content_items.append(
        {
            "type": "paragraph",
            "text": letter_content["bureau_name"],
            "bold": True,
            "size": 11,
        }
    )
    for line in letter_content["bureau_address"].split("\n"):
        if line.strip():
            content_items.append(
                {"type": "paragraph", "text": line.strip(), "bold": False, "size": 11}
            )
    ...
    content_items.append(
        {
            "type": "paragraph",
            "text": "RE: Request for Security Freeze on Credit File",
            "bold": True,
            "size": 12,
        }
    )
    ...
    content_items.append(
        {"type": "paragraph", "text": intro_text, "bold": False, "size": 11}
    )
    content_items.append({"type": "spacing"})
    ...
    content_items.append(
        {
            "type": "paragraph",
            "text": "CONSUMER IDENTIFICATION INFORMATION:",
            "bold": True,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": f"Full Legal Name: {letter_content['client_name']}",
            "bold": False,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": f"Current Address: {letter_content['client_address']}",
            "bold": False,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": f"Date of Birth: {letter_content['dob']}",
            "bold": False,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": f"SSN (Last 4 digits): XXX-XX-{letter_content['ssn_last4']}",
            "bold": False,
            "size": 11,
        }
    )
    ...
    content_items.append(
        {"type": "paragraph", "text": "LEGAL AUTHORITY:", "bold": True, "size": 11}
    )
    legal_text = (
        "Under FCRA Section 605A, as amended by the Economic Growth, Regulatory Relief, and Consumer "
        "Protection Act of 2018, consumers have the right to place, temporarily lift, or permanently "
    ...
    content_items.append(
        {"type": "paragraph", "text": legal_text, "bold": False, "size": 11}
    )
    content_items.append({"type": "spacing"})

    content_items.append(
        {"type": "paragraph", "text": "REQUEST:", "bold": True, "size": 11}
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": "I hereby request that you:",
            "bold": False,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": "1. Place a security freeze on my credit file immediately upon receipt of this letter.",
            "bold": False,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": "2. Provide me with written confirmation that the freeze has been placed, including any PIN, password, or other authentication method I will need to temporarily lift or permanently remove the freeze in the future.",
            "bold": False,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": "3. Send the confirmation to my address listed above within the timeframe required by law.",
            "bold": False,
            "size": 11,
        }
    )
    ...
    content_items.append(
        {"type": "paragraph", "text": "ENCLOSURES:", "bold": True, "size": 11}
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": "1. Copy of government-issued photo identification (driver's license or state ID)",
            "bold": False,
            "size": 11,
        }
    )
    content_items.append(
        {
            "type": "paragraph",
            "text": "2. Proof of current address (utility bill, bank statement, or similar document)",
            "bold": False,
            "size": 11,
        }
    )
    ...
    content_items.append(
        {"type": "paragraph", "text": notice_text, "bold": False, "size": 11}
    )
    content_items.append({"type": "spacing"})
    ...
    content_items.append(
        {
            "type": "paragraph",
            "text": f"Date: {letter_content['date']}",
            "bold": False,
            "size": 11,
        }
    )
...
def generate_freeze_letters(client_id, bureaus=None):
    ...
    try:
        ...
        for bureau_name in target_bureaus:
            ...
            letter_content = generate_single_freeze_letter(
                client, bureau_name, bureau_info
            )
...
def get_freeze_batch_status(batch_uuid):
    ...
    try:
        batch = (
            ...
            .filter(FreezeLetterBatch.batch_uuid == batch_uuid)
            .first()
        )
        if not batch:
...
def list_client_freeze_batches(client_id):
    ...
    try:
        batches = (
            db.query(FreezeLetterBatch)
            .filter(FreezeLetterBatch.client_id == client_id)
