...
class StripePlansService:
    ...
    def _get_integration_id(self, db: Any) -> Optional[int]:
        ...
        integration = (
            db.query(IntegrationConnection).filter_by(service_name="stripe").first()
        )
    ...
    def create_checkout_session(
        self, client_id: int, plan_id: int, success_url: str, cancel_url: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self._log_event(
                ...
                {"client_id": client_id, "plan_id": plan_id, "error": str(e)},
                client_id=client_id,
                response_status=500,
                error_message=str(e),
            )
    ...
    def create_customer_portal_session(
        ...
        try:
            ...
            session = self.stripe.billing_portal.Session.create(
                customer=client.stripe_customer_id, return_url=return_url
            )

            self._log_event(
                db,
                "portal_session_created",
                {"client_id": client_id, "portal_url": session.url},
                client_id=client_id,
            )
        ...
        try:
            ...
            if webhook_secret:
                ...
                event = self.stripe.Event.construct_from(
                    event_data, self.stripe.api_key
                )
    ...
    def _handle_checkout_completed(self, db, session) -> Dict[str, Any]:
        ...
        subscription = (
            db.query(ClientSubscription).filter_by(client_id=int(client_id)).first()
        )
    ...
    def _handle_subscription_created(self, db, subscription) -> Dict[str, Any]:
        ...
        client_sub = db.query(ClientSubscription).filter_by(client_id=client.id).first()
        ...
        client_sub.current_period_start = datetime.fromtimestamp(
            subscription.current_period_start
        )
        client_sub.current_period_end = datetime.fromtimestamp(
            subscription.current_period_end
        )
        client_sub.next_payment_date = datetime.fromtimestamp(
            subscription.current_period_end
        )
    ...
    def _handle_invoice_paid(self, db, invoice) -> Dict[str, Any]:
        ...
        if client_sub:
            client_sub.amount_paid_cents = (
                client_sub.amount_paid_cents or 0
            ) + invoice.amount_paid
            client_sub.updated_at = datetime.utcnow()
            db.commit()
    ...
    def get_subscription_status(self, client_id: int) -> Dict[str, Any]:
        ...
        try:
            subscription = (
                db.query(ClientSubscription).filter_by(client_id=client_id).first()
            )
            ...
            if subscription.plan_id:
                plan_obj = (
                    db.query(BillingPlan).filter_by(id=subscription.plan_id).first()
                )
                if plan_obj:
                    plan = {
                        "id": plan_obj.id,
    ...
    def initialize_default_plans(self) -> Dict[str, Any]:
        ...
        for plan_config in self.DEFAULT_PLANS:
            ...
            try:
                existing = (
                    db.query(BillingPlan).filter_by(name=plan_config["name"]).first()
                )
            ...
            if result.get("success"):
                ...
                try:
                    plan = (
                        ...
                        .filter_by(name=plan_config["name"])
                        .first()
                    )
                    if plan:
    ...
    def get_subscription_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            active_subscriptions = (
                db.query(ClientSubscription).filter_by(status="active").count()
            )
            canceled_subscriptions = (
                db.query(ClientSubscription).filter_by(status="canceled").count()
            )
            ...
            total_revenue = (
                db.query(func.sum(ClientSubscription.amount_paid_cents)).scalar() or 0
            )
    ...
    def get_active_subscriptions(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            subscriptions = (
                db.query(ClientSubscription)
                .filter(ClientSubscription.status.in_(["active", "past_due"]))
            ...
            for sub in subscriptions:
                ...
                plan = (
                    db.query(BillingPlan).filter_by(id=sub.plan_id).first()
                    if sub.plan_id
                    else None
                )
                ...
                result.append(
                    {
                        ...
                        "plan_price": (
                            f"${plan.price_cents / 100:.2f}" if plan else None
                        ),
                        "status": sub.status,
                        "current_period_end": (
