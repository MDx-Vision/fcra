...
ONBOARDING_STEPS = [
    {
        ...
        'icon': 'user',
        'required_fields': ['first_name', 'last_name', 'email', 'date_of_birth', 'address_street', 'address_city', 'address_state', 'address_zip']
    },
    {
        'key': 'id_documents',
...
class OnboardingService:
    ...
    def get_or_create_progress(self, client_id: int) -> Optional[OnboardingProgress]:
        ...
        progress = self.db.query(OnboardingProgress).filter(
            OnboardingProgress.client_id == client_id
        ).first()
    ...
    def _check_step_completion(self, client: Client, progress: OnboardingProgress, step_key: str) -> bool:
        ...
        if step_key == 'personal_info':
            ...
            uploads = self.db.query(ClientUpload).filter(
                ClientUpload.client_id == client.id,
                ClientUpload.document_type.in_(['drivers_license', 'drivers_license_front', 'id_front', 'dl_front'])
            ).count()
            return uploads > 0
            ...
            uploads = self.db.query(ClientUpload).filter(
                ClientUpload.client_id == client.id,
                ClientUpload.document_type.in_(['ssn_card', 'ssn_card_front', 'social_security'])
            ).count()
            return uploads > 0
            ...
            uploads = self.db.query(ClientUpload).filter(
                ClientUpload.client_id == client.id,
                ClientUpload.document_type.in_(['utility_bill', 'proof_of_address', 'bank_statement'])
            ).count()
            return uploads > 0

        elif step_key == 'credit_monitoring':
            # Check for credit monitoring credentials
            return bool(client.credit_monitoring_service and client.credit_monitoring_username)
    ...
    def _update_overall_progress(self, progress: OnboardingProgress):
        ...
        progress.completion_percentage = int((completed_count / len(ONBOARDING_STEPS)) * 100)
    ...
    def get_all_incomplete(self, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        for progress in incomplete:
            client = self.db.query(Client).filter(Client.id == progress.client_id).first()
            if client:
                results.append({
                    'client_id': client.id,
                    'client_name': f"{client.first_name or ''} {client.last_name or ''}".strip() or client.name or 'Unknown',
                    'email': client.email,
                    'percentage': progress.completion_percentage,
