...
from database import (
    WhiteLabelTenant, TenantUser, TenantClient,
    Staff, Client, Case, Analysis, SUBSCRIPTION_TIERS
)
...
class WhiteLabelService:
    ...
    def create_tenant(self, name: str, slug: str, settings: Optional[Dict[str, Any]] = None) -> WhiteLabelTenant:
        """
        ...
        if settings.get('domain'):
            existing_domain = self.db.query(WhiteLabelTenant).filter_by(domain=settings['domain']).first()
            if existing_domain:
                raise ValueError(f"Tenant with domain '{settings['domain']}' already exists")
    ...
    def update_tenant(self, tenant_id: int, **kwargs) -> Optional[WhiteLabelTenant]:
        ...
        if 'slug' in kwargs and kwargs['slug'] != tenant.slug:
            existing = self.db.query(WhiteLabelTenant).filter(
        ...
        if 'domain' in kwargs and kwargs['domain'] and kwargs['domain'] != tenant.domain:
            existing = self.db.query(WhiteLabelTenant).filter(
                WhiteLabelTenant.domain == kwargs['domain'],
                WhiteLabelTenant.id != tenant_id
            ).first()
            if existing:
                raise ValueError(f"Tenant with domain '{kwargs['domain']}' already exists")
        
        allowed_fields = [
            'name', 'slug', 'domain', 'logo_url', 'favicon_url',
            'primary_color', 'secondary_color', 'accent_color',
            'company_name', 'company_address', 'company_phone',
            'company_email', 'support_email', 'terms_url', 'privacy_url',
            'custom_css', 'custom_js', 'is_active', 'subscription_tier',
            'max_users', 'max_clients', 'features_enabled', 'webhook_url'
        ]
    ...
    def get_tenant_by_slug(self, slug: str) -> Optional[WhiteLabelTenant]:
        ...
        return self.db.query(WhiteLabelTenant).filter_by(slug=slug, is_active=True).first()
    
    def get_tenant_by_domain(self, domain: str) -> Optional[WhiteLabelTenant]:
        """Get a tenant by custom domain"""
        if not domain:
            return None
        return self.db.query(WhiteLabelTenant).filter_by(domain=domain, is_active=True).first()
    
    def get_tenant_by_api_key(self, api_key: str) -> Optional[WhiteLabelTenant]:
        """Get a tenant by API key"""
        if not api_key:
            return None
        return self.db.query(WhiteLabelTenant).filter_by(api_key=api_key, is_active=True).first()
    ...
    def assign_user_to_tenant(self, staff_id: int, tenant_id: int, role: str = 'user', is_primary_admin: bool = False) -> TenantUser:
        """
        ...
        existing = self.db.query(TenantUser).filter_by(
            tenant_id=tenant_id,
            staff_id=staff_id
        ).first()
    ...
    def remove_user_from_tenant(self, staff_id: int, tenant_id: int) -> bool:
        ...
        tenant_user = self.db.query(TenantUser).filter_by(
            tenant_id=tenant_id,
            staff_id=staff_id
        ).first()
    ...
    def assign_client_to_tenant(self, client_id: int, tenant_id: int) -> TenantClient:
        ...
        current_count = self.db.query(TenantClient).filter_by(tenant_id=tenant_id).count()
        if current_count >= tenant.max_clients:
            raise ValueError(f"Tenant has reached maximum client limit ({tenant.max_clients})")
        
        existing = self.db.query(TenantClient).filter_by(
            tenant_id=tenant_id,
            client_id=client_id
        ).first()
    ...
    def remove_client_from_tenant(self, client_id: int, tenant_id: int) -> bool:
        ...
        tenant_client = self.db.query(TenantClient).filter_by(
            tenant_id=tenant_id,
            client_id=client_id
        ).first()
    ...
    def get_user_tenants(self, staff_id: int) -> List[WhiteLabelTenant]:
        ...
        return self.db.query(WhiteLabelTenant).filter(
            WhiteLabelTenant.id.in_(tenant_ids),
            WhiteLabelTenant.is_active == True
        ).all()
    
    def get_client_tenant(self, client_id: int) -> Optional[WhiteLabelTenant]:
        """Get the tenant a client belongs to"""
        tenant_client = self.db.query(TenantClient).filter_by(client_id=client_id).first()
        if not tenant_client:
            return None
    ...
    def validate_tenant_features(self, tenant_id: int, feature_name: str) -> bool:
        ...
        tier_config = SUBSCRIPTION_TIERS.get(tenant.subscription_tier, SUBSCRIPTION_TIERS['basic'])
    ...
    def get_tenant_usage_stats(self, tenant_id: int) -> Dict[str, Any]:
        ...
        client_count = self.db.query(TenantClient).filter_by(tenant_id=tenant_id).count()
        ...
        if client_ids:
            case_count = self.db.query(Case).filter(Case.client_id.in_(client_ids)).count()
            analysis_count = self.db.query(Analysis).filter(Analysis.client_id.in_(client_ids)).count()
        ...
        return {
            ...
            'clients': {
                ...
                'usage_percent': round((client_count / tenant.max_clients) * 100, 1) if tenant.max_clients > 0 else 0
            },
            'cases': case_count,
