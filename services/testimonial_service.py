...
from database import Client, ClientTestimonial, get_db
...
def create_testimonial_request(client_id: int) -> Dict[str, Any]:
    ...
    if existing:
        return {
            "success": False,
            "error": "Client already has pending testimonial request",
        }
...
def submit_testimonial(
    token: str, rating: int, testimonial_text: str, video_url: str = None
) -> Dict[str, Any]:
    """Submit a testimonial via the review link"""
    db = get_db()
    testimonial = (
        db.query(ClientTestimonial)
        .filter(ClientTestimonial.request_token == token)
        .first()
    )
...
def list_testimonials(
    status: str = None, min_rating: int = None, limit: int = 50, offset: int = 0
) -> Dict[str, Any]:
    ...
    total = query.count()
    testimonials = (
        query.order_by(desc(ClientTestimonial.submitted_at))
        .offset(offset)
        .limit(limit)
        .all()
    )
    ...
    return {"testimonials": result, "total": total, "limit": limit, "offset": offset}
...
def approve_testimonial(
    ...
    testimonial = (
        db.query(ClientTestimonial)
        .filter(ClientTestimonial.id == testimonial_id)
        .first()
    )
...
def get_public_testimonials(
    limit: int = 10, featured_only: bool = False
) -> List[Dict[str, Any]]:
...
def get_testimonial_stats() -> Dict[str, Any]:
    ...
    total = (
        db.query(ClientTestimonial)
        .filter(ClientTestimonial.submitted_at.isnot(None))
        .count()
    )

    pending = (
        db.query(ClientTestimonial)
        .filter(ClientTestimonial.status == "pending")
        .count()
    )

    approved = (
        db.query(ClientTestimonial)
        .filter(ClientTestimonial.status.in_(["approved", "featured"]))
        .count()
    )
    ...
    avg_rating = (
        db.query(func.avg(ClientTestimonial.rating))
        .filter(ClientTestimonial.submitted_at.isnot(None))
        .scalar()
    )
