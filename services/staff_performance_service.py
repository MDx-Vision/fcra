...
from database import SessionLocal, Staff, Client, StaffActivity, Case, ClientMessage
...
class StaffPerformanceService:
    ...
    def mark_escalated(self, activity_id: int) -> Tuple[bool, str]:
        ...
        session = self._get_session()
        try:
            activity = session.query(StaffActivity).filter(
                StaffActivity.id == activity_id
            ).first()
            if not activity:
    ...
    def get_staff_metrics(
        ...
        try:
            ...
            activities = session.query(StaffActivity).filter(
                and_(
            ...
            response_times = [
                a.response_time_minutes for a in activities
            ...
            # Quality metrics
            quality_scores = [
                a.quality_score for a in activities
            ...
            return {
                ...
                'response_time': {
                    ...
                    'average_hours': round(avg_response_time / 60, 1) if avg_response_time else 0,
                ...
                'quality': {
                    ...
                    'revisions': revisions,
                    'escalation_rate': round(escalations / total_activities * 100, 1) if total_activities else 0,
                },
    ...
    def get_recent_team_activities(self, limit: int = 20) -> List[Dict]:
        ...
        session = self._get_session()
        try:
            activities = session.query(StaffActivity).order_by(
                StaffActivity.created_at.desc()
            ).limit(limit).all()
    ...
    def get_dashboard_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Today's stats
            today_activities = session.query(StaffActivity).filter(
                StaffActivity.created_at >= today_start
            ).all()
            today_points = sum(
                ACTIVITY_TYPES.get(a.activity_type, {}).get('points', 0)
            ...
            # This week's stats
            week_activities = session.query(StaffActivity).filter(
                StaffActivity.created_at >= week_start
            ).all()
            week_points = sum(
                ACTIVITY_TYPES.get(a.activity_type, {}).get('points', 0)
            ...
            # This month's stats
            month_activities = session.query(StaffActivity).filter(
                StaffActivity.created_at >= month_start
            ).all()
            month_points = sum(
                ACTIVITY_TYPES.get(a.activity_type, {}).get('points', 0)
            ...
            response_times = [
                a.response_time_minutes for a in month_activities
                if a.response_time_minutes is not None
            ]
            avg_response_time = (
                sum(response_times) / len(response_times)
