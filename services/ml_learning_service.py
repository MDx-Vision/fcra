...
from database import SessionLocal, CaseOutcome, OutcomePrediction, Violation, Standing, Damages, CaseScore, Client, Analysis, Furnisher, FurnisherStats, Staff
import json
import math
...
class MLLearningService:
    ...
    def record_outcome(self, client_id: int, outcome_data: dict) -> dict:
        ...
        try:
            outcome = CaseOutcome(
                ...
                dispute_rounds_completed=outcome_data.get('dispute_rounds_completed', 0),
    ...
    def _update_predictions_with_actual(self, db, client_id: int, outcome: CaseOutcome):
        ...
        predictions = db.query(OutcomePrediction).filter(
        ...
        for pred in predictions:
            if pred.prediction_type == 'outcome':
                ...
                try:
                    ...
                    pred.was_accurate = pred.prediction_error / max(outcome.settlement_amount, 1) < 0.25
                except:
                    pass
                ...
                try:
                    predicted = float(pred.predicted_value)
                    pred.prediction_error = abs(predicted - outcome.time_to_resolution_days)
    ...
    def get_similar_cases(self, violation_types: list = None, furnisher_id: int = None, 
                          limit: int = 10) -> list:
        """
    ...
    def calculate_success_rate(self, filters: dict = None) -> dict:
        ...
        try:
            ...
            if filters.get('furnisher_id'):
                query = query.filter(CaseOutcome.furnisher_id == filters['furnisher_id'])
    ...
    def generate_prediction_features(self, client_id: int) -> dict:
        ...
        try:
            ...
            case_score = db.query(CaseScore).filter(CaseScore.client_id == client_id).first()
            ...
            features = {
                ...
                'willfulness_ratio': sum(1 for v in violations if v.is_willful) / max(len(violations), 1),
            ...
            if damages:
                ...
                features['total_damages'] = features['actual_damages'] + features['statutory_damages']
            
            if case_score:
                features['case_score'] = case_score.total_score or 0
                features['settlement_probability'] = case_score.settlement_probability or 0
            
            weighted_score = 0
            for vtype in features['violation_types']:
                vtype_lower = vtype.lower() if vtype else ''
                weight = self.VIOLATION_WEIGHTS.get(vtype_lower, self.VIOLATION_WEIGHTS['default'])
                weighted_score += weight
            features['weighted_violation_score'] = round(weighted_score, 2)
            
            features['has_reinsertion'] = any('reinsertion' in (v or '').lower() for v in features['violation_types'])
            features['has_identity_theft'] = any('identity' in (v or '').lower() for v in features['violation_types'])
    ...
    def predict_outcome(self, client_id: int) -> dict:
        ...
        try:
            ...
            base_confidence = min(0.9, success_data.get('confidence', 0.5) + (features['violation_count'] * 0.02))
    ...
    def update_model_accuracy(self) -> dict:
        ...
        try:
            predictions = db.query(OutcomePrediction).filter(
                OutcomePrediction.actual_value.isnot(None)
            ).all()
            ...
            for ptype in by_type:
                data = by_type[ptype]
                data['accuracy_rate'] = round(data['accurate'] / max(data['total'], 1), 3)
                if data['errors']:
                    data['mean_error'] = round(sum(data['errors']) / len(data['errors']), 2)
                else:
    ...
    def get_learning_stats(self) -> dict:
        ...
        try:
            ...
            resolved_predictions = db.query(OutcomePrediction).filter(
                OutcomePrediction.actual_value.isnot(None)
            ).count()
            ...
            predictions_last_30 = db.query(OutcomePrediction).filter(
                OutcomePrediction.created_at >= datetime.utcnow() - timedelta(days=30)
            ).count()
            
            predictions_last_7 = db.query(OutcomePrediction).filter(
                OutcomePrediction.created_at >= datetime.utcnow() - timedelta(days=7)
            ).count()
            ...
            return {
                ...
                'model_info': {
                    ...
                    'features': [
                        'violation_count', 'violation_types', 'willfulness_ratio',
                        'concrete_harm', 'denial_letters', 'bureaus_count',
                        'case_score', 'settlement_probability'
                    ],
...
def get_similar_cases(violation_types: list = None, furnisher_id: int = None, limit: int = 10) -> list:
