...
class AutoPullService:
    ...
    def add_credential(
        ...
        try:
            ...
            if existing:
                return {
                    "success": False,
                    "error": "Active credential already exists for this service",
                }
    ...
    def update_credential(
        ...
        try:
            credential = (
                db.query(CreditMonitoringCredential)
                .filter(CreditMonitoringCredential.id == credential_id)
                .first()
            )
            ...
            if import_frequency:
                ...
                if import_frequency in FREQUENCIES and FREQUENCIES[import_frequency]:
                    credential.next_scheduled_import = (
                        datetime.utcnow() + FREQUENCIES[import_frequency]
                    )
    ...
    def get_credentials(
        self, client_id: int = None, service_name: str = None, active_only: bool = True
    ) -> List[Dict]:
        ...
        try:
            ...
            credentials = query.order_by(
                CreditMonitoringCredential.created_at.desc()
            ).all()
    ...
    def initiate_pull(
        self, credential_id: int, pull_type: str = "manual", triggered_by: str = None
    ) -> Dict[str, Any]:
    ...
    def _execute_pull(
        self, credential: CreditMonitoringCredential, pull_log: CreditPullLog
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if result.get("success"):
                ...
                if (
                    credential.import_frequency in FREQUENCIES
                    and FREQUENCIES[credential.import_frequency]
                ):
                    credential.next_scheduled_import = (
                        datetime.utcnow() + FREQUENCIES[credential.import_frequency]
                    )
    ...
    def _simulate_pull(
        self, credential: CreditMonitoringCredential, password: str
    ) -> Dict[str, Any]:
        ...
        report_filename = (
            f"report_{credential.client_id}_{credential.service_name}_{timestamp}.html"
        )
    ...
    def trigger_letter_send_pulls(
        self, client_id: int, triggered_by: str = "letter_send"
    ) -> Dict[str, Any]:
    ...
    def get_pull_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_credentials = (
                db.query(CreditMonitoringCredential)
                .filter(CreditMonitoringCredential.is_active == True)
                .count()
            )
            ...
            pulls_today = (
                db.query(CreditPullLog)
                .filter(CreditPullLog.initiated_at >= today_start)
                .count()
            )
            ...
            pending_scheduled = (
                ...
                .filter(
                    CreditMonitoringCredential.is_active == True,
                    CreditMonitoringCredential.import_frequency != "manual",
                    CreditMonitoringCredential.next_scheduled_import
                    <= datetime.utcnow(),
                )
                .count()
            )
            ...
            pulls_this_week = (
                db.query(CreditPullLog)
                .filter(CreditPullLog.initiated_at >= week_start)
                .count()
            )

            # Average pull duration
            avg_duration = (
                db.query(func.avg(CreditPullLog.duration_seconds))
                .filter(
                    CreditPullLog.status == STATUS_SUCCESS,
    ...
    def validate_credentials(self, credential_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "valid": is_valid,
                "message": (
                    "Credentials validated" if is_valid else "Invalid credentials"
                ),
            }
