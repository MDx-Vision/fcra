...
from database import BillingPlan, Client, ClientSubscription
...
class SubscriptionService:
    ...
    def sync_plans_to_stripe(self) -> Dict[str, Any]:
        ...
        for plan_key, plan_config in SUBSCRIPTION_PLANS.items():
            ...
            db_plan = (
                self.db.query(BillingPlan).filter(BillingPlan.name == plan_key).first()
            )
    ...
    def get_plan_by_name(self, name: str) -> Optional[BillingPlan]:
        """Get a plan by name."""
        return self.db.query(BillingPlan).filter(BillingPlan.name == name).first()
    ...
    def get_or_create_stripe_customer(self, client: Client) -> str:
        ...
        subscription = (
            self.db.query(ClientSubscription)
        ...
        if customers.data:
            ...
            customer = stripe.Customer.create(
                ...
                or client.email,
                metadata={"client_id": str(client.id), "source": "fcra_platform"},
            )
    ...
    def update_customer_payment_method(
        self, client_id: int, payment_method_id: str
    ) -> Dict[str, Any]:
        """
    ...
    def create_checkout_session(
        self, client_id: int, plan_name: str, success_url: str, cancel_url: str
    ) -> Dict[str, Any]:
        """
    ...
    def create_subscription(
        self, client_id: int, plan_name: str, payment_method_id: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def cancel_subscription(
        self, client_id: int, at_period_end: bool = True, reason: Optional[str] = None
    ) -> Dict[str, Any]:
        """
    ...
    def reactivate_subscription(self, client_id: int) -> Dict[str, Any]:
        ...
        subscription = stripe.Subscription.modify(
            db_subscription.stripe_subscription_id, cancel_at_period_end=False
        )
    ...
    def get_subscription(self, client_id: int) -> Optional[Dict[str, Any]]:
        ...
        if db_subscription.plan_id:
            plan = (
                ...
                .filter(BillingPlan.id == db_subscription.plan_id)
                .first()
            )
        ...
        return {
            ...
            "current_period_start": (
                db_subscription.current_period_start.isoformat()
                if db_subscription.current_period_start
                else None
            ),
            "current_period_end": (
                db_subscription.current_period_end.isoformat()
                if db_subscription.current_period_end
                else None
            ),
            "cancel_at_period_end": db_subscription.cancel_at_period_end,
            "canceled_at": (
                db_subscription.canceled_at.isoformat()
    ...
    def create_billing_portal_session(
        self, client_id: int, return_url: str
    ) -> Dict[str, Any]:
        """
    ...
    def _handle_checkout_completed(self, session: Dict) -> Dict[str, Any]:
        ...
        self._save_subscription(
            int(client_id), plan.id if plan else None, subscription, customer_id
        )
    ...
    def _handle_invoice_paid(self, invoice: Dict) -> Dict[str, Any]:
        ...
        db_subscription.amount_paid_cents = (
            db_subscription.amount_paid_cents or 0
        ) + amount_paid
        db_subscription.status = "active"

        # Update next payment date
        if invoice.get("next_payment_attempt"):
            db_subscription.next_payment_date = datetime.fromtimestamp(
    ...
    def _handle_invoice_payment_failed(self, invoice: Dict) -> Dict[str, Any]:
        ...
        # Send dunning email
        client = (
            self.db.query(Client).filter(Client.id == db_subscription.client_id).first()
        )
        if client and client.email:
            from services.email_service import send_subscription_past_due_email

            send_subscription_past_due_email(
                client_email=client.email, client_name=client.name or "Client"
            )
    ...
    def _save_subscription(
        ...
        db_subscription.stripe_customer_id = customer_id or (
        ...
        db_subscription.cancel_at_period_end = (
            ...
            else subscription.get("cancel_at_period_end", False)
        )
