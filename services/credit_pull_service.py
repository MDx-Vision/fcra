...
from database import SessionLocal, CreditPullRequest, IntegrationEvent, IntegrationConnection, Client, Analysis, CreditReport
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    } if data.get('status') == 'complete' else None,
...
class ExperianConnectAdapter(BaseCreditAdapter):
    ...
    def _get_access_token(self) -> Optional[str]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self._token_expires_at = datetime.utcnow().timestamp() + data.get('expires_in', 3600) - 60
                return self._access_token
        except Exception as e:
    ...
    def is_configured(self) -> bool:
        ...
        return bool(
            (self.api_key and len(self.api_key) > 10) or
    ...
    def request_report(
        ...
        payload = {
            'consumerPii': {
                'primaryApplicant': {
                    ...
                    'ssn': {
                        'ssn': full_ssn or ('XXXXX' + ssn_last4)
                    }
                }
            },
            'requestor': {
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                return {
                    'success': True,
                    'request_id': data.get('reportId') or data.get('transactionId') or str(uuid.uuid4()),
...
class CreditPullService:
    ...
    def set_provider(self, provider_name: str, api_key: Optional[str] = None) -> bool:
        ...
        if provider_name not in PROVIDER_ADAPTERS:
            logger.error(f"Unknown provider: {provider_name}. Available: {list(PROVIDER_ADAPTERS.keys())}")
            return False
    ...
    def request_credit_report(
        ...
        try:
            ...
            logger.info(f"Requesting credit report for client {client_id} via {self.provider_name}")
    ...
    def get_report_status(self, request_id: int) -> Dict[str, Any]:
        ...
        session = SessionLocal()
        try:
            pull_request = session.query(CreditPullRequest).filter_by(id=request_id).first()
            if not pull_request:
                return {
                    'success': False,
                    'status': 'not_found',
            ...
            if status_result.get('success') and status_result.get('status') == 'complete':
    ...
    def get_client_pulls(self, client_id: int) -> List[Dict[str, Any]]:
        ...
        session = SessionLocal()
        try:
            pulls = session.query(CreditPullRequest).filter_by(
                client_id=client_id
            ).order_by(CreditPullRequest.created_at.desc()).all()
    ...
    def import_to_analysis(self, request_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            credit_report = CreditReport(
                ...
                report_html=str(pull_request.parsed_data) if pull_request.parsed_data else None,
    ...
    def _get_required_env_vars(self, provider: str) -> List[str]:
        ...
        env_vars = {
            ...
            'experian_connect': ['EXPERIAN_CLIENT_ID', 'EXPERIAN_CLIENT_SECRET', 'EXPERIAN_SUBSCRIBER_CODE']
        }
