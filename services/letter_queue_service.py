...
def queue_letter(db: Session, 
    ...
    try:
        existing = db.query(LetterQueue).filter_by(
            client_id=client_id,
...
def check_cra_response_triggers(db: Session, cra_response_id: int) -> List[Dict]:
    ...
    """
    from database import CRAResponse, DisputeItem, Client
    ...
    if response.response_type == 'verified':
        dispute_items = db.query(DisputeItem).filter_by(
    ...
    if response.response_type == 'reinserted' or (response.structured_items and 
        any(item.get('reinserted') for item in response.structured_items if isinstance(item, dict))):
...
def check_no_response_triggers(db: Session, days_threshold: int = 35) -> List[Dict]:
    ...
    """
    from database import DisputeItem, Client, CRAResponse
    
    results = []
    cutoff_date = datetime.utcnow() - timedelta(days=days_threshold)
    
    pending_items = db.query(DisputeItem).filter(
    ...
    for item in pending_items:
        has_response = db.query(CRAResponse).filter(
...
def check_item_type_triggers(db: Session, dispute_item_id: int) -> List[Dict]:
    ...
    """
    from database import DisputeItem, Client
    ...
    if item.item_type == 'late_payment' and item.creditor_name and any(
        keyword in (item.creditor_name or '').lower() 
        for keyword in ['mortgage', 'home loan', 'fannie', 'freddie', 'wells fargo home', 
                       'bank of america home', 'chase home', 'quicken', 'rocket mortgage']
    ):
        result = queue_letter(
            db=db,
            client_id=item.client_id,
...
def check_escalation_triggers(db: Session, dispute_item_id: int, new_stage: str) -> List[Dict]:
    """
...
def get_pending_queue(db: Session, 
    ...
    """
    from database import LetterQueue, Client, DisputeItem
    ...
    for entry in entries:
        ...
        dispute_item = db.query(DisputeItem).filter_by(id=entry.dispute_item_id).first() if entry.dispute_item_id else None
        ...
        results.append({
            ...
            'letter_type_display': LETTER_TYPE_DISPLAY.get(entry.letter_type, entry.letter_type),
            'trigger_type': entry.trigger_type,
            'trigger_display': TRIGGER_DISPLAY.get(entry.trigger_type, entry.trigger_type),
            'trigger_description': entry.trigger_description,
            'trigger_date': entry.trigger_date.isoformat() if entry.trigger_date else None,
...
def get_queue_stats(db: Session) -> Dict:
    ...
    total = db.query(func.count(LetterQueue.id)).filter(LetterQueue.status == 'pending').scalar() or 0
    
    by_priority = {}
    for priority in ['urgent', 'high', 'normal', 'low']:
        count = db.query(func.count(LetterQueue.id)).filter(
            LetterQueue.status == 'pending',
            LetterQueue.priority == priority
        ).scalar() or 0
    ...
    for letter_type in LETTER_TYPE_DISPLAY.keys():
        count = db.query(func.count(LetterQueue.id)).filter(
            LetterQueue.status == 'pending',
            LetterQueue.letter_type == letter_type
        ).scalar() or 0
        if count > 0:
            by_letter_type[letter_type] = count
    
    approved_today = db.query(func.count(LetterQueue.id)).filter(
        LetterQueue.status == 'approved',
        LetterQueue.reviewed_at >= datetime.utcnow().replace(hour=0, minute=0, second=0)
    ).scalar() or 0
    
    generated_today = db.query(func.count(LetterQueue.id)).filter(
        LetterQueue.status == 'generated',
        LetterQueue.generated_at >= datetime.utcnow().replace(hour=0, minute=0, second=0)
    ).scalar() or 0
...
def approve_queue_entry(db: Session, 
                       queue_id: int, 
                       staff_id: int,
...
def dismiss_queue_entry(db: Session,
                       queue_id: int,
                       staff_id: int,
                       reason: str) -> Dict:
    """
