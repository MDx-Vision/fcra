...
from services.chat_knowledge_base import check_escalation_needed, get_system_prompt
...
class ChatService:
    ...
    def send_message(
        self, conversation_id: int, message: str, client_id: int = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_conversation(
        self, conversation_id: int, client_id: int = None
    ) -> Dict[str, Any]:
        """
    ...
    def get_client_conversations(
        self, client_id: int, include_closed: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def get_escalated_conversations(
        self, staff_id: int = None, include_resolved: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        conversations = query.order_by(ChatConversation.escalated_at.desc()).all()
            ...
            result.append(
                {
                    ...
                    "status": conv.status,
                    "escalated_at": (
                        conv.escalated_at.isoformat() if conv.escalated_at else None
                    ),
    ...
    def staff_respond(
        self, conversation_id: int, staff_id: int, message: str
    ) -> Dict[str, Any]:
        """
        ...
        if conversation.status not in ["escalated", "active"]:
            return {
                "success": False,
                "error": f"Cannot respond to {conversation.status} conversation",
            }
    ...
    def get_client_context(self, client_id: int) -> Dict[str, Any]:
        ...
        items_removed = (
            self.db.query(func.count(DisputeItem.id))
            .filter(DisputeItem.client_id == client_id, DisputeItem.status == "deleted")
            .scalar()
            or 0
        )
