...
class ClientPaymentService:
    ...
    def create_analysis_payment_intent(
        self, client_id: int, return_url: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def create_round_payment_intent(
        self, client_id: int, round_number: int = None
    ) -> Dict[str, Any]:
        """
    ...
    def confirm_round_payment(
        self, client_id: int, payment_intent_id: str, round_number: int
    ) -> Dict[str, Any]:
        """
        ...
        log_activity(
            "Confirm Payment",
            f"Round {round_number} payment confirmation",
            client_id=client_id,
            status="info",
        )
        try:
            ...
            if round_number == 1:
                amount = (
                    ROUND_1_WITH_CREDIT if client.analysis_paid_at else ROUND_1_PRICE
                )
            else:
            ...
            if self.stripe:
                ...
                log_activity(
                    "Verify with Stripe",
                    f"Checking payment intent {payment_intent_id[:20]}...",
                    client_id=client_id,
                    status="info",
                )
                intent = self.stripe.PaymentIntent.retrieve(payment_intent_id)
                if intent.status != "succeeded":
                    log_payment_failed(
                        client_id, amount / 100, f"Stripe status: {intent.status}"
                    )
                    return {
                        "success": False,
                        "error": f"Payment not successful: {intent.status}",
            ...
            log_payment_success(
                client_id, amount / 100, f"Round {round_number} - Stripe"
            )
    ...
    def charge_for_round(
        self, client_id: int, round_number: int, payment_method_id: str = None
    ) -> Dict[str, Any]:
        """
    ...
    def confirm_prepay_package(
        self, client_id: int, session_id: str, package_key: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
                ...
                payments.append(
                    {
                        ...
                        "amount": ANALYSIS_PRICE,
                        "date": (
                            client.analysis_paid_at.isoformat()
                            if client.analysis_paid_at
                            else None
                        ),
                        "status": "paid",
            ...
            return {
                ...
                "next_payment_display": (
                    "Prepaid" if has_prepaid_rounds else f"${next_amount/100:.2f}"
                ),
                ...
                "analysis_credit_amount": (
                    ANALYSIS_PRICE if client.analysis_paid_at else 0
                ),
            }
    ...
    def save_payment_method(
        ...
        try:
            ...
            if (
                not hasattr(client, "stripe_customer_id")
                or not client.stripe_customer_id
            ):
            ...
            self.stripe.PaymentMethod.attach(payment_method_id, customer=customer.id)
