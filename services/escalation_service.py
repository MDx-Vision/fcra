...
from database import (
    get_db, Client, Analysis, Violation, Damages, Standing, CRAResponse,
    DisputeItem, Furnisher, FurnisherStats, EscalationRecommendation, CaseScore
)
...
def get_dispute_history(db, client_id: int) -> List[Dict]:
    ...
    cra_responses = db.query(CRAResponse).filter_by(client_id=client_id).order_by(CRAResponse.created_at.desc()).all()
    ...
    dispute_items = db.query(DisputeItem).filter_by(client_id=client_id).order_by(DisputeItem.created_at.desc()).all()
...
def get_furnisher_performance(db, creditor_name: str) -> Dict:
    ...
    furnisher = db.query(Furnisher).filter(Furnisher.name.ilike(f"%{creditor_name}%")).first()
    ...
    total_round1 = stats.round_1_verified + stats.round_1_deleted + stats.round_1_updated
    ...
    return {
        ...
        'settlement_rate': stats.settlement_count / stats.total_disputes if stats.total_disputes > 0 else 0.1,
        'settlement_avg': stats.settlement_avg or 0,
        'violation_count': stats.violation_count or 0,
        'mov_success_rate': stats.mov_failed / stats.mov_requests_sent if stats.mov_requests_sent > 0 else 0.5,
...
def calculate_violation_severity(db, client_id: int, analysis_id: int = None) -> Dict:
    ...
    for v in violations:
        ...
        severity['section_counts'][section] = severity['section_counts'].get(section, 0) + 1
    
    severity['severity_score'] = min(10, (
...
def recommend_escalation(client_id: int, item_id: int = None, bureau: str = None) -> Dict:
    """
    ...
    try:
        ...
        latest_analysis = db.query(Analysis).filter_by(client_id=client_id).order_by(Analysis.created_at.desc()).first()
        ...
        bureaus_to_check = ['Experian', 'TransUnion', 'Equifax'] if not bureau else [bureau]
        ...
        return {
            ...
            'summary': {
                ...
                'highest_confidence': max(r['confidence_score'] for r in recommendations) if recommendations else 0,
                'primary_action': recommendations[0]['recommended_action'] if recommendations else None
            }
        }
...
def _generate_recommendation(
    ...
    if frivolous_count > 0:
        ...
        reasoning = f"No response within 30-day statutory period - clear FCRA §611 violation."
    ...
    current_status = 'verified' if verified_count > 0 else 'investigating' if dispute_round > 1 else 'new'
...
def get_escalation_timeline(client_id: int) -> Dict:
    ...
    try:
        ...
        last_response = db.query(CRAResponse).filter_by(client_id=client_id).order_by(CRAResponse.created_at.desc()).first()
        last_letter_date = last_response.created_at if last_response else client.round_started_at or client.created_at
        ...
        if recommendations.get('success') and recommendations.get('recommendations'):
            ...
            if primary_rec['recommended_action'] in ['wait_response', 'standard_redispute']:
                ...
                timeline.append({
                    ...
                    'priority': 'high' if deadline <= today + timedelta(days=5) else 'medium'
                })
            ...
            if primary_rec['recommended_action'] in ['intent_to_sue', 'settlement_demand']:
                timeline.append({
        ...
        past_recs = db.query(EscalationRecommendation).filter_by(
            client_id=client_id, applied=True
        ).order_by(EscalationRecommendation.applied_at.desc()).limit(5).all()
        
        for rec in past_recs:
            timeline.append({
                'date': rec.applied_at.isoformat() if rec.applied_at else rec.created_at.isoformat(),
...
def record_outcome(recommendation_id: int, outcome: str) -> Dict:
    ...
    try:
        ...
        if rec.creditor_name:
            _update_furnisher_stats(db, rec.creditor_name, rec.recommended_action, outcome)
...
def get_escalation_stats() -> Dict:
    ...
    try:
        ...
        for rec in completed_recs:
            ...
            action_stats[action]['outcomes'][outcome] = action_stats[action]['outcomes'].get(outcome, 0) + 1
        ...
        return {
            ...
            'overall_success_rate': sum(s['successful'] for s in action_stats.values()) / len(completed_recs) if completed_recs else 0
        }
    finally:
        db.close()
...
def get_cases_needing_escalation_review() -> List[Dict]:
    ...
    try:
        ...
        for client in clients:
            last_response = db.query(CRAResponse).filter_by(
                client_id=client.id
            ).order_by(CRAResponse.created_at.desc()).first()
            ...
            if last_response:
                days_since_response = (datetime.utcnow() - last_response.created_at).days
                
                if last_response.response_type == 'verified' and days_since_response > 7:
                    needs_review = True
                    ...
                    priority = 'urgent'
                elif last_response.response_type == 'no_response' or days_since_response > 35:
                    needs_review = True
                    reason = 'Response deadline passed'
                    priority = 'urgent'
            else:
                if client.round_started_at:
                    days_since_start = (datetime.utcnow() - client.round_started_at).days
            ...
            if needs_review:
                cases_needing_review.append({
                    ...
                    'reason': reason,
                    'priority': priority,
                    'last_response_date': last_response.created_at.isoformat() if last_response else None,
                    'last_response_type': last_response.response_type if last_response else None
                })
        
        cases_needing_review.sort(
            key=lambda x: {'urgent': 0, 'high': 1, 'medium': 2, 'low': 3}.get(x['priority'], 2)
        )
