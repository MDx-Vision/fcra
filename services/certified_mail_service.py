...
from database import CertifiedMailOrder, SessionLocal
...
def send_certified_letter(
    ...
    try:
        ...
        if not is_certified_mail_configured():
            ...
            logger.info(
                f"Created mock certified mail order {order.id} for client {client_id}"
            )
        ...
        with open(document_path, "rb") as f:
            files = {
                "document": (os.path.basename(document_path), f, "application/pdf")
            }
        ...
        logger.info(
            f"Created certified mail order {order.id} with tracking {tracking_number}"
        )
...
def check_delivery_status(order_id):
    ...
    try:
        order = (
            ...
            .filter(CertifiedMailOrder.id == order_id)
            .first()
        )
        ...
        if not is_certified_mail_configured():
            logger.warning(
                "SENDCERTIFIEDMAIL_API_KEY not configured - cannot check live status"
            )
            return {
                "success": True,
                "order_id": order_id,
        ...
        logger.info(
            f"Checking status for order {order_id}, external ID: {order.external_order_id}"
        )
        ...
        if normalized_status == "mailed" and not order.mailed_at:
            mailed_str = response_data.get("mailed_at") or response_data.get(
                "mail_date"
            )
            if mailed_str:
                try:
                    order.mailed_at = datetime.fromisoformat(
                        mailed_str.replace("Z", "+00:00")
                    )
                except:
        ...
        if normalized_status == "delivered" and not order.delivered_at:
            delivered_str = response_data.get("delivered_at") or response_data.get(
                "delivery_date"
            )
            if delivered_str:
                try:
...
def handle_webhook(webhook_data):
    ...
    try:
        external_order_id = (
            ...
            or webhook_data.get("external_order_id")
        )
        event_type = (
        ...
        logger.info(
            f"Received webhook for order {external_order_id}, event: {event_type}"
        )
        ...
        order = (
            session.query(CertifiedMailOrder)
            .filter(CertifiedMailOrder.external_order_id == external_order_id)
            .first()
        )
        ...
        if new_status == "delivered":
            if not order.delivered_at:
                delivered_str = webhook_data.get("delivered_at") or webhook_data.get(
                    "delivery_date"
                )
                if delivered_str:
                    try:
            ...
            proof_url = (
                webhook_data.get("delivery_proof_url")
                or webhook_data.get("return_receipt_url")
                or webhook_data.get("proof_url")
            )
            if proof_url:
...
def get_orders_by_client(client_id):
    ...
    try:
        orders = (
            session.query(CertifiedMailOrder)
            .filter(CertifiedMailOrder.client_id == client_id)
            .order_by(CertifiedMailOrder.created_at.desc())
            .all()
        )
...
def get_pending_orders():
    ...
    try:
        ...
        orders = (
            session.query(CertifiedMailOrder)
            .filter(CertifiedMailOrder.status.in_(pending_statuses))
            .order_by(CertifiedMailOrder.created_at.asc())
            .all()
        )
